...
if ($keyb['id_cost']=="") {
    ...
    $pdf->Text(35, 20, 'Pialang Asuransi');
        ...
        if ($met['dn_status']=="unpaid") {
            $headtagihan = $pdf->Text(97, 30, 'KWITANSI');
        } else {
            $headtagihan = $pdf->Text(88, 30, 'TAGIHAN PREMI');
        }
...
*/
        ...
        if ($tanggalplus > $datelog) {
            $pdf->cell(30, 7, '', 0, 0, 'L', 1);
            $pdf->cell(80, 7, '', 0, 0, 'L', 1);
            $pdf->cell(1, 7, '', 0, 0, 'L', 1);
            $pdf->cell(75, 7, 'Jakarta, '.$futgl.'', 0, 0, 'L', 1);
        } else {
            $pdf->cell(30, 7, 'Keterangan', 0, 0, 'L', 1);
            $pdf->cell(80, 7, ': ', 0, 0, 'L', 1);
            $pdf->cell(1, 7, '', 0, 0, 'L', 1);
            $pdf->cell(75, 7, 'Jakarta, '.$futgl.'', 0, 0, 'L', 1);
        }
...
if ($tanggalplus > $datelog or $met['dn_status']=="paid" or $met['dn_status']!="paid") {
    $pdf->cell(111, 7, '', 0, 0, 'L', 1);
    $pdf->cell(75, 7, 'PT. ADONAI PIALANG ASURANSI', 0, 0, 'L', 1);
    if ($_REQUEST['ats']=="adm") {
...
if ($metpolis['logoasuransi']=="Y") {
    ...
    $pdf->Image('image/adonai_64.gif', 10, 18);
    $pdf->SetFont('Arial', '', 8);
    $pdf->Text(10, 40, 'Nama Penanggung');
    $pdf->Text(50, 40, ': '.$met_asuransi['name'].' ('.$metpolis['nokontrak'].')');
}
...
if ($metpolis['nmproduk']=="") {
    $metproduknya = $metpolis['nopol'];
} else {
    $metproduknya = $metpolis['nmproduk'];
}
        $pdf->SetFont('Arial', '', 8);
...
*/
    ...
    */
    ...
    //JML KARAKTER
    if (strlen($metpeserta['id_peserta'] <= 8)) {
        $idpesertanya = substr($metpeserta['id_peserta'], 11);
    } else {
        $idpesertanya = $metpeserta['id_peserta'];
    }
...
case "_erKlaim":
    ...
    $met_klaim = mysql_fetch_array(mysql_query('SELECT *,DATEDIFF(approve_date,tgl_claim)as diftoday,DATE_ADD(tgl_claim,INTERVAL 120 DAY)as date_exp FROM fu_ajk_cn WHERE id="'.$_REQUEST['idC'].'" AND del IS NULL'));
    ...
    if ($met_klaim_peserta['spaj'] != "") {
        $pdf->Text(125, 50, 'No Formulir');
        $pdf->Text(165, 50, ': '.$met_klaim_peserta['spaj']);
    }
    ...
    //
    // $diff = abs(strtotime($met_klaim['tgl_claim']) - strtotime($met_klaim_peserta['tgl_lahir']));
    // function fn_selisih_waktu($tgl1, $tgl2)
    // {
    //     $tgl = strtotime($tgl1);
    //     $skrg = strtotime($tgl2);
    //     $diff_secs = abs($skrg - $tgl);
    //     $base_year = min(date("Y", $skrg), date("Y", $tgl));
    //     $diff = mktime(0, 0, $diff_secs, 1, 1, $base_year);
    //     return array(
    //       "tahun"=>date("Y", $diff) - $base_year,
    //       "bulan"=>date("n", $diff)-1,
    //       "total_hari"=>floor($diff_secs/(3600*24)),
    //       "hari"=>date("j", $diff)-1,
    //     );
    // }

    // $gdr = fn_selisih_waktu($met_klaim_peserta['tgl_lahir'], $met_klaim['tgl_claim']);
	...
	WHERE fu_ajk_klaim_doc.id_klaim = '.$met_klaim['id'].' and
...
if ($met['confirm_claim']=="Approve(paid)") {
    $headtagihan = $pdf->Text(80, 30, 'PEMBAYARAN REFUND');
} else {
    $headtagihan = $pdf->Text(85, 30, 'KWITANSI REFUND');
}

if ($met['tgl_byr_claim'] =="") {
    $tglbayarCN = ' -';
} else {
    $tglbayarCN = _convertDate($met['tgl_byr_claim']);
}
...
$metdn = mysql_fetch_array(mysql_query('SELECT id, dn_kode FROM fu_ajk_dn WHERE id="'.$met['id_dn'].'"'));
if ($met['confirm_claim']=="Approve(paid)") {
    $headtagihan = $pdf->Text(80, 30, 'PEMBAYARAN BATAL');
} else {
    $headtagihan = $pdf->Text(85, 30, 'KWITANSI ' .strtoupper($met['type_claim']));
}
...
$pdf=new FPDF('P', 'mm', 'A4');
        ...
        if (substr($met['spak'], 0, 2)=="MP") {
            $pdf->SetFont('helvetica', 'B', 12);
            $pdf->Text(80, 30, 'FORMULIR PERCEPATAN');
            $pdf->SetFont('helvetica', 'B', 12);
            $pdf->Text(90, 35, 'Nomor '.$met['spak'].' ');
            $tenornya = $metForm['tenor'] * 12;
        //$tenornya = $metForm['tenor'];
        } elseif (substr($met['spak'], 0, 2)=="MS") {
            $pdf->SetFont('helvetica', 'B', 12);
            $pdf->Text(80, 30, 'FORMULIR PASANGAN');
            $pdf->SetFont('helvetica', 'B', 12);
            $pdf->Text(88, 35, 'Nomor. '.$met['spak']);
            $tenornya = $metForm['tenor'];
        } elseif (substr($met['spak'], 0, 2)=="PL") {
            $pdf->SetFont('helvetica', 'B', 12);
            $pdf->Text(81, 30, 'FORMULIR PLATINUM');
            $pdf->SetFont('helvetica', 'B', 12);
            $pdf->Text(88, 35, 'Nomor. '.$met['spak']);
            $tenornya = $metForm['tenor'] * 12;
        } else {
            $pdf->SetFont('helvetica', 'B', 12);
            $pdf->Text(60, 30, 'SURAT PEMERIKSAAN KESEHATAN "SPK"');
            $pdf->SetFont('helvetica', 'B', 12);
            $pdf->Text(95, 35, 'SPK. '.$met['spak']);
            $tenornya = $metForm['tenor'];
        }
        $metFormSPK = mysql_fetch_array(mysql_query('SELECT * FROM fu_ajk_spak_form WHERE idspk="'.$met['id'].'" AND del IS NULL'));
        if ($metFormSPK['jns_kelamin']=="M") {
            $gender = "Laki-Laki";
        } else {
            $gender = "Perempuan";
        }
        $metCekDokter = mysql_fetch_array(mysql_query('SELECT id, namalengkap, type, level FROM user_mobile WHERE id="'.$metFormSPK['dokter_pemeriksa'].'"'));
...
if (is_numeric($metdata_form['cabang'])) {
    ...
    $inputcabang = $metFormSPK['cabang'];
        ...
        if ($met_polis['mpptype']=='Y') {
            ...
            $sety = '55';
            $pdf->Text(10, 50, 'Nama Produk');
            $pdf->Text(52, 50, ': '.$met_polis['nmproduk'].' ');
            $pdf->Text(10, $sety, 'MPP Bulan (Grace Period)');
            $pdf->Text(52, $sety, ': '.$metFormSPK['mpp'].' Bulan');
            $sety ='60';
        } else {
            $sety = '55';
            $pdf->Text(10, 50, 'Nama Produk');
...
if ($metdata_form['filefotodebiturdua']!="") {
    ...
    if ($qruser['id_cost']!="") {
        ...
        if ($metdata_form['filettddokter']!="") {
            ...
            if (file_exists($pathFile)) {
                ...
                $pdf->Text(115, 270, 'Tidak Ada TTD Dokter');
            }
        } else {
            $pdf->Text(115, 270, 'Tidak Ada TTD Dokter');
        }
    ...
    if ($_cekSPK=="MP" or $_cekSPK=="PL" or $_cekSPK=="MS") {
        ...
        if ($met['status']=="Tolak") {
            $statusSPK = 'Ditolak';
        } elseif ($met['status']=="Batal") {
            $statusSPK = 'Dibatalkan';
        } else {
            $statusSPK = $met['status'];
        }
        ...
        if ($_REQUEST['mod']!="adn") {
            ...
            $pdf->SetFont('helvetica', 'U', 9);
            $pdf->SetX(9);
            $pdf->MultiCell(195, 4, 'DATA CEK MEDICAL', 0, 'L');

            $pdf->SetFont('helvetica', '', 9);

            if ($metCekDokter['type']=="Dokter" and $metCekDokter['level']=="Klinik") {
                $dokter = 	', '.$metFormSPK['dokter_pemeriksa'];
            }
            $pdf->SetX(9);
            $pdf->Cell(36, 4, 'Dokter Pemeriksa', 0, 'L');
            $pdf->Cell(36, 4, ': '.strtoupper($metCekDokter['namalengkap']).''.$dokter, 0, 'l');
            $pdf->Ln();
            $pdf->SetX(9);
            $pdf->Cell(36, 4, 'Tanggal Periksa', 0, 'L');
            $pdf->Cell(36, 4, ': ' ._convertDate($metFormSPK['tgl_periksa']), 0, 'l');
            $pdf->Ln();

            $pdf->SetX(10);
            $pdf->Cell(37, 5, 'Tinggi dan Berat Badan', 1, 0, 'C');
            ...
            $pdf->Ln();
            $pdf->SetX(10);
            $pdf->Cell(37, 5, $metFormSPK['tinggibadan'].'/'.$metFormSPK['beratbadan'].'', 1, 0, 'C');
            ...
            $pdf->Ln();
            $pdf->SetX(9);
            $pdf->MultiCell(195, 4, 'Kesimpulan : '.$metFormSPK['kesimpulan'].'', 0, 'L');
            $pdf->SetX(9);
            $pdf->MultiCell(195, 4, 'Catatan : '.$metFormSPK['catatan'].'', 0, 'L');


            $pdf->Ln();
            $pdf->SetFont('helvetica', 'U', 9);
            $pdf->SetX(9);
            $pdf->MultiCell(195, 4, 'DATA PEMINJAMAN ASURANSI', 0, 'L');
            $pdf->SetFont('helvetica', '', 9);
            $pdf->SetX(10);
            ...
            if ($met['status']=="Aktif" or $met['status']=="Realisasi" or $met['status']=="Preapproval" or $met['status']=="Kadaluarsa") {
                if ($met['ext_premi']=="") {
                    $met_ket_EM ='PREMI STANDAR';
                } else {
                    $met_ket_EM = 'EM '.$met['ext_premi'].'%  ('.$met['ket_ext'].')';
                }
                ...
                if ($_REQUEST['fo']=="as") {
                    $cekData_ = mysql_fetch_array(mysql_query('SELECT fu_ajk_peserta.id_dn,
																	  ...
																	  INNER JOIN fu_ajk_asuransi ON fu_ajk_asuransi.id = fu_ajk_dn.id_as
            ...
            $pdf->Ln();
            $pdf->SetX(9);
            $pdf->Cell(37, 4, 'Cabang', 0, 'L');
            $pdf->Cell(36, 4, ': ' .$inputcabang, 0, '1');
            $pdf->SetX(9);
            $pdf->Cell(37, 4, 'Input User', 0, 'L');
            $pdf->Cell(36, 4, ': ' .strtoupper($inputby_met), 0, '1');
            $pdf->SetX(9);
            $pdf->Cell(37, 4, 'Tanggal Input', 0, 'L');
            $pdf->Cell(36, 4, ': ' .$metFormSPK['input_date'], 0, '1');

            // Hansen 20180516 REV18050040
            if ($_REQUEST['fo']=="as") {
                $pdf->SetX(9);
                $pdf->Cell(37, 4, 'Asuransi', 0, 'L');
                $pdf->Cell(36, 4, ': '.$cekData_['name'].' ('.$cekData_['code'].')', 0, '1');
            }
            //END Hansen REV18050040
            ...
            if ($statususer=="" or $statususer=="STAFF" or $statususer=="SUPERVISOR") {
                ...
                $pdf->Ln();
                //if ($met['status']=="Aktif" OR $met['status']=="Realisasi") { ditambahkan status yang preaproval untuk melihat data keterangan plafondnya 29092016
                if ($met['status']=="Aktif" or $met['status']=="Realisasi" or $met['status']=="Preapproval" or $met['status']=="Batal") {
                    if ($met['ext_premi']=="") {
                        $met_ket_EM ='PREMI STANDAR';
                    } else {
                        $met_ket_EM = 'EM '.$met['ext_premi'].'%  ('.$met['ket_ext'].')';
                    ...
                    if ($met['status']=="Preapproval") {
                        ...
                        //$pdf->Cell(30,5,duit($mettotalpremibayangan_) ,1,0,'C');
                        if ($_REQUEST['fo']=="as") {
                            $premias = mysql_fetch_array(mysql_query("SELECT nettpremi
																												FROM fu_ajk_peserta_as
																												WHERE id_peserta = (SELECT id_peserta
																																						FROM fu_ajk_peserta
																																						WHERE SPAJ = '".$met['spak']."')"));
                            $pdf->Cell(30, 5, duit($premias['nettpremi']), 1, 0, 'C');
                        } else {
                            $pdf->Cell(30, 5, duit($mettotalpremibayangan_), 1, 0, 'C');
                        }
                    }
                    ...
                    if ($cekSKKT['question_2']) {
                        ...
                        $pdf->Cell(190, 5, 'KETERANGAN KESEHATAN TERTANGGUNG', 0, 0, 'C');
                        $pdf->Ln();
                        $pdf->Ln();
                        $pdf->SetFont('Arial', '', 10);
                        ...
                        $pdf->Cell(140, 5, 'Apakah Anda dalam keadaan sehat / tidak sehat :', 0, 0, 'L');
                        $pdf->Cell(20, 5, 'Sehat', 0, 0, 'C');
                        $pdf->Cell(25, 5, 'Tidak Sehat', 0, 0, 'C');
                        $pdf->Ln();
                        $pdf->SetFont('Arial', '', 10);
                        if ($metSehat[0]=="S") {
                            $setkolom1 = 'Sehat';
                        } else {
                            $setkolom11 = 'Tidak Sehat';
                        }
                        $pdf->Cell(6, 5, '', 0, 0, 'L');
                        $pdf->Cell(5, 5, 'a. ', 0, 0, 'L');
                        $pdf->Cell(135, 5, 'Pada saat ini dalam keadaan :', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom1, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom11, 0, 0, 'C');
                        $pdf->Ln();

                        if ($metSehat[1]=="S") {
                            $setkolom2 = 'Sehat';
                        } else {
                            $setkolom22 = 'Tidak Sehat';
                        }
                        $pdf->Cell(6, 5, '', 0, 0, 'L');
                        $pdf->Cell(5, 5, 'b. ', 0, 0, 'L');
                        $pdf->Cell(135, 5, 'Biasanya dalam keadaan :', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom2, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom22, 0, 0, 'C');
                        $pdf->Ln();
                        $pdf->Ln();
                        ...
                        $pdf->Cell(5, 5, 'a. ', 0, 0, 'L');
                        $pdf->Cell(135, 5, 'Dalam jangka waktu 2 tahun terakhir ini, apakah anda pernah / tidak pernah :', 0, 0, 'L');
                        $pdf->Cell(20, 5, 'Pernah', 0, 0, 'C');
                        $pdf->Cell(25, 5, 'Tidak Pernah', 0, 0, 'C');
                        $pdf->Ln();
                        ...
                        if ($metSehat[0]=="P") {
                            ...
                            $setkolom33 = 'Tidak Pernah';
                        }
                        $pdf->Cell(10, 5, '', 0, 0, 'L');
                        $pdf->Cell(7, 5, '1. ', 0, 0, 'L');
                        $pdf->Cell(129, 5, 'Menderita penyakit malaria ', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom3, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom33, 0, 0, 'C');
                        $pdf->Ln();

                        if ($metSehat[1]=="P") {
                            $setkolom4 = 'Pernah';
                        } else {
                            $setkolom44 = 'Tidak Pernah';
                        }
                        $pdf->Cell(10, 5, '', 0, 0, 'L');
                        $pdf->Cell(7, 5, '2. ', 0, 0, 'L');
                        $pdf->Cell(129, 5, 'Menderita penyakit kanker ', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom4, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom44, 0, 0, 'C');
                        $pdf->Ln();

                        if ($metSehat[2]=="P") {
                            $setkolom5 = 'Pernah';
                        } else {
                            $setkolom55 = 'Tidak Pernah';
                        }
                        $pdf->Cell(10, 5, '', 0, 0, 'L');
                        $pdf->Cell(7, 5, '3. ', 0, 0, 'L');
                        $pdf->Cell(129, 5, 'Menderita penyakit TBC ', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom5, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom55, 0, 0, 'C');
                        $pdf->Ln();

                        if ($metSehat[3]=="P") {
                            $setkolom6 = 'Pernah';
                        } else {
                            $setkolom66 = 'Tidak Pernah';
                        }
                        $pdf->Cell(10, 5, '', 0, 0, 'L');
                        $pdf->Cell(7, 5, '4. ', 0, 0, 'L');
                        $pdf->Cell(129, 5, 'Menderita penyakit kencing manis ', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom6, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom66, 0, 0, 'C');
                        $pdf->Ln();

                        if ($metSehat[4]=="P") {
                            $setkolom7 = 'Pernah';
                        } else {
                            $setkolom77 = 'Tidak Pernah';
                        }
                        $pdf->Cell(10, 5, '', 0, 0, 'L');
                        $pdf->Cell(7, 5, '5. ', 0, 0, 'L');
                        $pdf->Cell(129, 5, 'Menderita penyakit hati ', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom7, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom77, 0, 0, 'C');
                        $pdf->Ln();

                        if ($metSehat[5]=="P") {
                            $setkolom8 = 'Pernah';
                        } else {
                            $setkolom88 = 'Tidak Pernah';
                        }
                        $pdf->Cell(10, 5, '', 0, 0, 'L');
                        $pdf->Cell(7, 5, '6. ', 0, 0, 'L');
                        $pdf->Cell(129, 5, 'Menderita penyakit ginjal ', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom8, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom88, 0, 0, 'C');
                        $pdf->Ln();

                        if ($metSehat[6]=="P") {
                            $setkolom9 = 'Pernah';
                        } else {
                            $setkolom99 = 'Tidak Pernah';
                        }
                        $pdf->Cell(10, 5, '', 0, 0, 'L');
                        $pdf->Cell(7, 5, '7. ', 0, 0, 'L');
                        $pdf->Cell(129, 5, 'Menderita penyakit jantung ', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom9, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom99, 0, 0, 'C');
                        $pdf->Ln();

                        if ($metSehat[7]=="P") {
                            $setkolom10 = 'Pernah';
                        } else {
                            $setkolom1010 = 'Tidak Pernah';
                        }
                        $pdf->Cell(10, 5, '', 0, 0, 'L');
                        $pdf->Cell(7, 5, '8. ', 0, 0, 'L');
                        $pdf->Cell(129, 5, 'Menderita penyakit ayan ', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom10, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom1010, 0, 0, 'C');
                        $pdf->Ln();

                        if ($metSehat[8]=="P") {
                            $setkolom11 = 'Pernah';
                        } else {
                            $setkolom1111 = 'Tidak Pernah';
                        }
                        $pdf->Cell(10, 5, '', 0, 0, 'L');
                        $pdf->Cell(7, 5, '9. ', 0, 0, 'L');
                        $pdf->Cell(129, 5, 'Menderita penyakit lumpuh ', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom11, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom1111, 0, 0, 'C');
                        $pdf->Ln();

                        if ($metSehat[9]=="P") {
                            $setkolom12 = 'Pernah';
                        } else {
                            $setkolom1212 = 'Tidak Pernah';
                        }
                        $pdf->Cell(10, 5, '', 0, 0, 'L');
                        $pdf->Cell(7, 5, '10. ', 0, 0, 'L');
                        $pdf->Cell(129, 5, 'Menderita penyakit syaraf ', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom12, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom1212, 0, 0, 'C');
                        $pdf->Ln();

                        if ($metSehat[10]=="P") {
                            $setkolom13 = 'Pernah';
                        } else {
                            $setkolom1313 = 'Tidak Pernah';
                        }
                        $pdf->Cell(10, 5, '', 0, 0, 'L');
                        $pdf->Cell(7, 5, '11. ', 0, 0, 'L');
                        $pdf->Cell(129, 5, 'Menderita penyakit tekanan darah tinggi ', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom13, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom1313, 0, 0, 'C');
                        $pdf->Ln();

                        if ($metSehat[11]=="P") {
                            $setkolom14 = 'Pernah';
                        } else {
                            $setkolom1414 = 'Tidak Pernah';
                        }
                        $pdf->Cell(10, 5, '', 0, 0, 'L');
                        $pdf->Cell(7, 5, '12. ', 0, 0, 'L');
                        $pdf->Cell(129, 5, 'Menderita penyakit tekanan darah rendah ', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom14, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom1414, 0, 0, 'C');
                        $pdf->Ln();

                        if ($metSehat[12]=="P") {
                            $setkolom15 = 'Pernah';
                        } else {
                            $setkolom1515 = 'Tidak Pernah';
                        }
                        $pdf->Cell(10, 5, '', 0, 0, 'L');
                        $pdf->Cell(7, 5, '13. ', 0, 0, 'L');
                        $pdf->Cell(129, 5, 'Menderita penyakit kelamin ', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom15, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom1515, 0, 0, 'C');
                        $pdf->Ln();

                        if ($metSehat[13]=="P") {
                            $setkolom16 = 'Pernah';
                        } else {
                            $setkolom1616 = 'Tidak Pernah';
                        }
                        $pdf->Cell(10, 5, '', 0, 0, 'L');
                        $pdf->Cell(7, 5, '14. ', 0, 0, 'L');
                        $pdf->Cell(129, 5, 'Dirawat di Rumah Sakit ', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom16, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom1616, 0, 0, 'C');
                        $pdf->Ln();

                        if ($metSehat[14]=="P") {
                            $setkolom17 = 'Pernah';
                        } else {
                            $setkolom1717 = 'Tidak Pernah';
                        }
                        $pdf->SetFont('Arial', 'B', 10);
                        $pdf->Cell(6, 5, '', 0, 0, 'L');
                        $pdf->Cell(5, 5, 'b. ', 0, 0, 'L');
                        $pdf->Cell(180, 5, 'Jika pernah dirawat di Rumah Sakit, sebutkan nama dan alamat Rumah Sakit', 0, 0, 'L');
                        $pdf->Ln();
                        $pdf->Cell(6, 5, '', 0, 0, 'L');
                        $pdf->Cell(5, 5, '', 0, 0, 'L');
                        $pdf->Cell(180, 5, 'yang merawat Anda', 0, 0, 'L');
                        $pdf->Ln();

                        $pdf->SetFont('Arial', '', 10);
                        $pdf->Cell(6, 5, '', 0, 0, 'L');
                        $pdf->Cell(5, 5, '', 0, 0, 'L');
                        $pdf->Cell(180, 5, $cekSKKT['nm_alamat_rs'], 0, 0, 'L');

                        if ($metSehat[15]=="P") {
                            $setkolom18 = 'Pernah';
                        } else {
                            $setkolom1818 = 'Tidak Pernah';
                        }
                        $pdf->Cell(6, 10, '', 0, 0, 'L');
                        $pdf->Cell(185, 10, ' ', 0, 0, 'L');
                        $pdf->Ln();

                        if ($cekSKKT['question_3']=="P") {
                            $setkolom19 = 'Pernah';
                        } else {
                            $setkolom1919 = 'Tidak Pernah';
                        }
                        $pdf->Cell(6, 5, '3. ', 0, 0, 'L');
                        $pdf->Cell(5, 5, 'a. ', 0, 0, 'L');
                        $pdf->Cell(135, 5, 'Dalam jangka waktu 12 bulan terakhir ini pernah / tidak pernah dirawat dokter:', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom19, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom1919, 0, 0, 'C');
                        $pdf->Ln();

                        if ($cekSKKT['nm_dokter']=="") {
                            $_ohhdokter = "---";
                        } else {
                            $_ohhdokter = $cekSKKT['nm_dokter'];
                        }
                        if ($cekSKKT['almt_dokter']=="") {
                            $_ohhdokter_ = "---";
                        } else {
                            $_ohhdokter_ = $cekSKKT['almt_dokter'];
                        }
                        $pdf->Cell(6, 5, '', 0, 0, 'L');
                        $pdf->Cell(5, 5, 'b. ', 0, 0, 'L');
                        $pdf->Cell(30, 5, 'Nama dokter ', 0, 0, 'L');
                        $pdf->Cell(150, 5, ': '.$_ohhdokter, 0, 0, 'L');
                        $pdf->Ln();
                        $pdf->Cell(6, 5, '', 0, 0, 'L');
                        $pdf->Cell(5, 5, '', 0, 0, 'L');
                        $pdf->Cell(30, 5, 'Alamat dokter ', 0, 0, 'L');
                        $pdf->Cell(150, 5, ': '.$_ohhdokter_, 0, 0, 'L');
                        $pdf->Ln();
                        $pdf->Ln();

                        $pdf->SetFont('Arial', 'B', 10);
                        if ($cekSKKT['question_4']=="H") {
                            $setkolom20 = 'Hamil';
                        } else {
                            $setkolom2020 = 'Tidak Hamil';
                        }

                        $pdf->SetFont('Arial', 'B', 10);
                        $pdf->Cell(6, 5, '4. ', 0, 0, 'L');
                        $pdf->Cell(140, 5, 'Saat ini dalam keadaan', 0, 0, 'L');
                        $pdf->Cell(20, 5, 'Hamil', 0, 0, 'C');
                        $pdf->Cell(25, 5, 'Tidak Hamil', 0, 0, 'C');
                        $pdf->Ln();

                        $pdf->SetFont('Arial', '', 10);
                        $pdf->Cell(6, 5, '', 0, 0, 'L');
                        $pdf->Cell(140, 5, '(hanya untuk wanita)', 0, 0, 'L');
                        $pdf->Cell(20, 5, $setkolom20, 0, 0, 'C');
                        $pdf->Cell(25, 5, $setkolom2020, 0, 0, 'C');
                        $pdf->Ln();
                        $pdf->Ln();
                        ...
                        $pdf->MultiCell(195, 4, 'Selanjutnya saya dengan ini memberi kuasa penuh kepada Pemegang Polis dan Dokter-Dokter yang akan atau telah memeriksa atau mengobati saya untuk memberi keterangan-keterangan yang diminta oleh Pihak Asuransi mengenai segala sesuatu yang diperlukan dalam hubungannya dengan penutupan asuransi ini.', 0, 'L');
                        $pdf->Ln();
                        $jangkrikgan = explode(" ", $cekSKKT['created_at']);
                        $pdf->Cell(190, 5, ucfirst(strtolower(($inputcabang))).', '. _convertDate($jangkrikgan[0]), 0, 0, 'R');
                        $pdf->Ln();
                        $pdf->Cell(190, 5, 'Mengetahui,', 0, 0, 'L');
                        $pdf->Ln();
                        $pdf->Cell(95, 5, 'Pejabat Bank / Koperasi / Lembaga', 0, 0, 'L');
                        $pdf->Cell(95, 5, 'Yang membuat pernyataan', 0, 0, 'C');
                        $pdf->Ln(32);
                        ...
                        if ($metdata_form['filettdmarketing']!="") {
                            ...
                            if (file_exists($pathFile)) {
                                ...
                                $pdf->Text(15, 240, 'Tidak Ada TTD Marketing');
                            }
                        }
                        ...
                        if ($metdata_form['filettddebitur']!="") {
                            ...
                            if (file_exists($pathFile)) {
                                ...
                                $pdf->Text(135, 240, 'Tidak Ada TTD Debitur');
                            }
                        }
...
if ($_REQUEST['st']) {
    $dua = 'AND fu_ajk_spak.status = "'.$_REQUEST['st'].'"';
}
...
if ($_REQUEST['r']=="tp") {
    ...
    $pdf->Text(20, 140, "Alasan Pembatalan");
    $pdf->Text(70, 140, ":");
    $pdf->Text(73, 140, "................................................................................................................");
    $pdf->Text(73, 145, "................................................................................................................");
    $pdf->Text(73, 150, "................................................................................................................");
...
while ($met_ = mysql_fetch_array($met)) {
    ...
    $pdf->Text(180, 5, 'Tanggal System '.$futgl);
    $pdf->SetFont('helvetica', 'B', 20);
    $pdf->SetTextColor(255, 0, 0);
    $pdf->Text(35, 15, 'A D O N A I');
    $pdf->SetFont('helvetica', '', 14);
    ...
    if (substr($met_['spak'], 0, 2)=="MP") {
        ...
        $pdf->Text(80, 30, 'FORMULIR PERCEPATAN');
        $pdf->SetFont('helvetica', 'B', 12);
        $pdf->Text(90, 35, 'Nomor '.$met_['spak'].' ');
        $tenornya = $metForm['tenor'] * 12;
    //$tenornya = $metForm['tenor'];
    } else {
        $pdf->SetFont('helvetica', 'B', 12);
    ...
    $pdf->SetFont('helvetica', 'B', 9);
    $pdf->Text(10, 45, 'Nama Perusahaan');
    $pdf->Text(52, 45, ': '.$met_costumer['name']);
    if ($met_polis['nmproduk']=='SPK REGULER MPP' or $met_polis['nmproduk']=='PERCEPATAN MPP') {
        $sety = '55';
        $pdf->Text(10, 50, 'Nama Produk');
        $pdf->Text(52, 50, ': '.$met_polis['nmproduk'].' ');
    ...
    $pdf->Text(10, $sety, 'Upload User');
    $pdf->Text(52, $sety, ': '.strtoupper($inputby_met));
    ...
    $pdf->Text(10, 70, 'Nama Nasabah');
    $pdf->Text(52, 70, ': '.strtoupper($metFormSPK['nama']));
    $pdf->Image($mamet_photonya, 160, 35, 30, 50);
    $pdf->Text(10, 75, 'Jenis Kelamin');
    $pdf->Text(52, 75, ': '.$gender);
    $pdf->Text(10, 80, 'Tanggal Lahir');
    $pdf->Text(52, 80, ': '._convertDate($metFormSPK['dob']));
    $pdf->Text(10, 85, 'Usia');
    $pdf->Text(52, 85, ': '.duit($metFormSPK['x_usia']).' Tahun');
    $pdf->Text(10, 90, 'Alamat');
    $pdf->Text(52, 90, ': '.nl2br($metFormSPK['alamat']));
    $pdf->Text(10, 95, 'Pekerjaan');
    $pdf->Text(52, 95, ': '.$metFormSPK['pekerjaan']);
    if ($met_['status']=="Tolak") {
        $statusSPK = 'Ditolak';
    } elseif ($met_['status']=="Batal") {
        $statusSPK = 'Dibatalkan';
    } else {
        $statusSPK = $met_['status'];
    }
    $pdf->Text(10, 100, 'Status');
    $pdf->Text(52, 100, ': '.$statusSPK);
    ...
    if ($met_['typeproduk']=="SPK") {
        ...
        $pdf->SetX(9);
        $pdf->Cell(36, 4, 'Dokter Pemeriksa', 0, 'L');
        $pdf->Cell(36, 4, ': '.strtoupper($metCekDokter['namalengkap']), 0, 'l');
        $pdf->Ln();
        $pdf->SetX(9);
        $pdf->Cell(36, 4, 'Tanggal Periksa', 0, 'L');
        $pdf->Cell(36, 4, ': ' ._convertDate($metFormSPK['tgl_periksa']), 0, 'l');
        ...
        if ($met_['status']=="Aktif" or $met_['status']=="Realisasi" or $met_['status']=="Preapproval") {
            if ($met_['ext_premi']=="") {
                ...
                $met_ket_EM = 'EM '.$met_['ext_premi'].'%  ('.$met_['ket_ext'].')';
            }
    ...
    case "outstandingpremi_bak20170529":
        ...
        while ($q_outstandingr = mysql_fetch_array($q_outstanding)) {
            ...
            $pdf->SetY($y);
            $pdf->SetX($x=$x+8);
            $pdf->cell(25, 5, $q_outstandingr['id_peserta'], 1, 0, 'C');
            $pdf->SetY($y);
            $pdf->SetX($x=$x+25);
            $pdf->cell(40, 5, $q_outstandingr['dn_kode'], 1, 0, 'C');
            $pdf->SetY($y);
            $pdf->SetX($x=$x+40);
            $pdf->cell(60, 5, $q_outstandingr['nama'], 1, 0, 'L');
            $pdf->SetY($y);
            $pdf->SetX($x=$x+60);
            $pdf->cell(22, 5, duit($q_outstandingr['kredit_jumlah']), 1, 0, 'R');
            $pdf->SetY($y);
            $pdf->SetX($x=$x+22);
            $pdf->cell(22, 5, $q_outstandingr['kredit_tgl'], 1, 0, 'C');
            //$pdf->SetY($y);$pdf->SetX($x=$x+20);$pdf->cell(20,5,$q_outstandingr['kredit_akhir'],1,0,'C');
            $pdf->SetY($y);
            $pdf->SetX($x=$x+22);
            $pdf->cell(23, 5, duit($q_outstandingr['totalpremi']), 1, 0, 'R');
    ...
    case "outstandingpremi":
        ...
        while ($q_outstandingr = mysql_fetch_array($q_outstanding)) {
            ...
            $pdf->SetY($y);
            $pdf->SetX($x=$x+7);
            $pdf->cell(16, 5, $q_outstandingr['id_peserta'], 1, 0, 'C');
            $pdf->SetY($y);
            $pdf->SetX($x=$x+16);
            $pdf->cell(30, 5, $q_outstandingr['dn_kode'], 1, 0, 'C');
            $pdf->SetY($y);
            $pdf->SetX($x=$x+30);
            $pdf->cell(45, 5, $q_outstandingr['nama'], 1, 0, 'L');
            $pdf->SetY($y);
            $pdf->SetX($x=$x+45);
            $pdf->cell(50, 5, $q_outstandingr['cabang'], 1, 0, 'L');
            $pdf->SetY($y);
            $pdf->SetX($x=$x+50);
            $pdf->cell(18, 5, duit($q_outstandingr['kredit_jumlah']), 1, 0, 'R');
            $pdf->SetY($y);
            $pdf->SetX($x=$x+18);
            $pdf->cell(15, 5, $q_outstandingr['kredit_tgl'], 1, 0, 'C');
            //$pdf->SetY($y);$pdf->SetX($x=$x+20);$pdf->cell(20,5,$q_outstandingr['kredit_akhir'],1,0,'C');
            $pdf->SetY($y);
            $pdf->SetX($x=$x+15);
            $pdf->cell(18, 5, duit($q_outstandingr['totalpremi']), 1, 0, 'R');
            $sumpremi = $sumpremi + $q_outstandingr['totalpremi'];
            if ($no==45) {
                $pdf->AddPage();
                $y = 5;
