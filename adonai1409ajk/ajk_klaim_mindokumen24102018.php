...
switch ($_REQUEST['d']) {
	case "mindokumen" :
		...
		$query = "
		SELECT fu_ajk_peserta.cabang,count(*)as jml 
		FROM fu_ajk_klaim 
		...
		while ($rklaim = mysql_fetch_array($metklaim)) {
			...
			<tr onmouseover="this.className=\'tbl-over\'" onmouseout="this.className=\'' . $objlass . '\'" class="' . $objlass . '">
				...
				<!--<td align="center"><a href="ajk_klaim_mindokumen.php?er=klaimkurangdokumen&cab='.$rklaim['cabang'].'"><img src="image/excel.png" width="22" height="22"></a></td>-->
				...
				<!--<td align="center"><a href="ajk_klaim_mindokumen.php?d=sendklaimkurangdokumen&cab='.$rklaim['cabang'].'"><img src="image/print.png" width="22" height="22"></a></td>-->
			</tr>';
		}
		echo '</table>';
	break;
	...
	case "previewsendklaimkurangdokumen":
		...
		$qcabang = mysql_query("select * from fu_ajk_cabang where name = '".$cab."' and del is null");
		if (mysql_num_rows($qcabang) > 0) {
			...
			if ($qcabang_row['id_reg'] == 15 or $qcabang_row['id_reg'] == 24 or $qcabang_row['id_reg'] == 17 or $qcabang_row['id_reg'] == 20 or $qcabang_row['id_reg'] == 19 or $qcabang_row['id_reg'] == 18) {
					...
					if ($qcabang_row['centralcbg']=="") {							
							$qemailto = mysql_query("select * from fu_ajk_email_cabang where id_cabang = (select id from fu_ajk_cabang where name = '".$cab."' and del is null)");
					} else {
							$qemailto = mysql_query("select * from fu_ajk_email_cabang where id_cabang = 4");
							$qemailtoaa = mysql_query("select * from fu_ajk_email_cabang where id_cabang = (select id from fu_ajk_cabang where name = '".$cab."' and del is null)");
					}
			}
		} else {
				$qemailto = mysql_query("select * from fu_ajk_email_cabang where id_cabang = 4");
	...
	case "sendklaimkurangdokumen" :
		...
		$mail->Subject = "[App AJK] Kekurangan Dokumen Klaim ".$cabang." Per ".$today; //Subject od your mail
		
		$qcabang = mysql_query("select * from fu_ajk_cabang where name = '".$cabang."' and del is null");
		if (mysql_num_rows($qcabang) > 0) {
			$qcabang_row = mysql_fetch_array($qcabang);
			if ($qcabang_row['id_reg'] == 15 or $qcabang_row['id_reg'] == 24 or $qcabang_row['id_reg'] == 17 or $qcabang_row['id_reg'] == 20 or $qcabang_row['id_reg'] == 19 or $qcabang_row['id_reg'] == 18) {
					$qemailto = mysql_query("select * from fu_ajk_email_cabang where id_cabang = (select name from fu_ajk_regional where id = '".$qcabang_row['id_reg']."')");
		...
		//$worksheet->write(1, 0, "PER ".$today, $format_center);
		...
		$query = "
		  ...
		  while ($row = mysql_fetch_array($result)) {
			...
			$worksheet->write($baris, 3, $row['id_peserta']);
			...
			$worksheet->write($baris, 16, $row['kadaluarsa']);        
			$baris++;
		  }
      ...
      $mail->SetFrom('klaim@adonai.co.id', 'Adonai Notification [no reply]');
      $mail->Subject = "[App AJK] Kekurangan Dokumen Klaim ".$cabang." Per ".$today; //Subject od your mail
      //$mail->AddAddress($tomail); //To address who will receive this email
      ...
      $mail->MsgHTML('Dear '.$cabang.',
			<br><br>Terlampir Rekap Klaim Kurang Dokumen '.$cabang.' Per '.$today.'.
			<br>Mohon Melengkapi dokumen klaim sebelum kadaluarsa (120 Hari setelah debitur meninggal dunia).
			<br><br>Atas Perhatian dan kerjasamanya, kami ucapakan terima kasih.
			<br><br><br>Hormat kami,<br><br><br>'.$q['nm_lengkap'].'<br><br>Telp : 021 8690 9090, Fax : 021 8690 8849
			<br><a href="#">klaim@adonai.co.id</a><br>');  
