...
*/
            ...
            //$mail->AddBCC("hansend@adonai.co.id");
...
echo '<tr onmouseover="this.className=\'tbl-over\'" onmouseout="this.className=\'' . $objlass . '\'" class="' . $objlass . '">
      ...
      if ($q['level'] == "99" AND $q['status'] == "STAFF") {
      	$query = 'SELECT *
                  ...
                  where (fu_ajk_spak_form.nama like "%'.$_REQUEST['namaspk'].'%" and
                         ...
                         ifnull(id_mitra,0) not in (23,26) and
                         danatalangan is NULL
        ...
        $query = 'SELECT fu_ajk_spak.*
                  ...
                  where (fu_ajk_spak_form.nama like "%'.$_REQUEST['namaspk'].'%" and
                         ...
                         fu_ajk_spak.del IS NULL and
                         fu_ajk_spak_form.del is null and
                         ifnull(id_mitra,0) not in (23,26) and danatalangan is NULL
                  ORDER BY fu_ajk_spak_form.tgl_periksa DESC';
      ...
      while($han_ = mysql_fetch_array($han)){
        ...
        if ($q['level'] == "99" AND $q['status'] == "STAFF"){
        	if($han_['status']=="Approve"){
          	...
          	$setting_fspak = '<a title="View" href="ajk_uploader_spak.php?r=vsett_spak&ids=' . $han_['id'] . '"><img src="image/new.png" width="20"></a> &nbsp;
          									  <a title="Edit" href="ajk_uploader_spak.php?r=edit_spak&ids=' . $han_['id'] . '&idform='.$han_formspk['id'].'"><img src="image/edit3.png" width="20"></a> &nbsp;';
    ...
    case "spak_assign":
      ...
      if ($_REQUEST['ope'] == "Simpan") {
        if($met_extpremi['nopermohonan'] != ""){
            $queryspk = mysql_query("select fu_ajk_spak.id
                                     from fu_ajk_spak
                                     inner join fu_ajk_spak_form
                                     on fu_ajk_spak_form.idspk = fu_ajk_spak.id
                                     where fu_ajk_spak_form.nopermohonan = '".$met_extpremi['nopermohonan']."' and id_polis != 19");
            while ($rqueryspk = mysql_fetch_array($queryspk)) {
              $s = $database->doQuery('UPDATE fu_ajk_spak
                                       SET  status = "Assign",
                                            assign_by="' . $q['nm_user'] . '",
                                            assign_date="' . $futgl . '",
                                            assign_note="' . $_REQUEST['assign_note'] . '"
                                       WHERE id="' . $rqueryspk['id'] . '"');
            }
        }else{
          $s = $database->doQuery('UPDATE fu_ajk_spak
                         SET  status = "Assign",
...
$nett_premi_spk = $_met_data_spk['x_premi'] + $nilai_premi_spk;
        if($_met_data_spk['nopermohonan'] != ""){
            $queryspk = mysql_query("select fu_ajk_spak.id
                                     from fu_ajk_spak
                                     inner join fu_ajk_spak_form
                                     on fu_ajk_spak_form.idspk = fu_ajk_spak.id
                                     where fu_ajk_spak_form.nopermohonan = '".$_met_data_spk['nopermohonan']."' and id_polis != 19");
            while ($rqueryspk = mysql_fetch_array($queryspk)) {
    ...
    case "extpremi_spk":
       ...
       if ($_REQUEST['ope'] == "Simpan") {
            ...
            if($q['status'] == "UNDERWRITING" and $_REQUEST['ext_premi'] > "50"){
              //$error2 .= '<blink><font color=red>Nilai extra premi harus di bawah 50%</font></blink><br>'; //REQUEST dr. diah 20180430
            }
            ...
            if ($error1 OR $error2) {
              ...
              if($met_extpremi['nopermohonan'] != ""){
                ...
                while ($rqueryspk = mysql_fetch_array($queryspk)) {
                  $s = $database->doQuery('UPDATE fu_ajk_spak
                                         SET  ext_premi="' . $_REQUEST['ext_premi'] . '",
                                              ket_ext="' . $met_ket . '",
                                              approve_em_by="' . $q['nm_lengkap'] . '",
                                              approve_em_date="' . $futgl . '"
                                          WHERE id="' . $rqueryspk['id'] . '"');
...
echo '<table border="0" width="100%" cellpadding="1" cellspacing="1" bgcolor="#bde0e6">
	 <tr><th width="1%">No</th>
         ...
         <th width="5%">User EM</th>
         <th width="5%">Tgl EM</th>
        ...
        if ($_REQUEST['namaspk']) 	{	$dua = 'AND nama LIKE "%' . $_REQUEST['namaspk'] . '%"';	}
...
$met = $database->doQuery('
	...
	WHERE (fu_ajk_spak.`status` = "Aktif" OR fu_ajk_spak.`status` = "Approve" OR fu_ajk_spak.`status` = "Tolak" OR fu_ajk_spak.`status` = "Realisasi" OR  fu_ajk_spak.`status` = "Kadaluarsa") AND
		   fu_ajk_spak.del IS NULL AND
    ...
    fu_ajk_spak_form_temp.noidentitas,
	...
	fu_ajk_spak.approve_em_date
	FROM fu_ajk_spak
	INNER JOIN fu_ajk_spak_form_temp ON fu_ajk_spak.id = fu_ajk_spak_form_temp.idspk
	INNER JOIN fu_ajk_polis ON fu_ajk_spak.id_polis = fu_ajk_polis.id
	LEFT JOIN fu_ajk_grupproduk ON fu_ajk_spak.id_mitra = fu_ajk_grupproduk.id
	INNER JOIN fu_ajk_costumer ON fu_ajk_spak.id_cost = fu_ajk_costumer.id
	WHERE (fu_ajk_spak.`status` = "Pending" OR fu_ajk_spak.`status` = "Proses") AND
		   fu_ajk_spak.del IS NULL AND
		   fu_ajk_polis.typeproduk ="SPK" AND
		   fu_ajk_spak_form_temp.del IS NULL ' . $satu . ' ' . $dua . '
	)as temp
	ORDER BY id DESC
	LIMIT ' . $m . ', 25');
	$totalRows = mysql_fetch_array($database->doQuery('SELECT COUNT(fu_ajk_spak.id)
	...
	INNER JOIN fu_ajk_costumer ON fu_ajk_spak.id_cost = fu_ajk_costumer.id
	WHERE (fu_ajk_spak.`status` = "Aktif" OR fu_ajk_spak.`status` = "Tolak" OR fu_ajk_spak.`status` = "Realisasi" OR  fu_ajk_spak.`status` = "Kadaluarsa"OR  fu_ajk_spak.`status` = "Pending"OR  fu_ajk_spak.`status` = "Approve") AND
		   fu_ajk_spak.del IS NULL AND
		   fu_ajk_polis.typeproduk ="SPK" AND
		   fu_ajk_spak_form_temp.del IS NULL ' . $satu . ' ' . $dua . ' '));
$totalRows = $totalRows[0];
...
echo '<tr onmouseover="this.className=\'tbl-over\'" onmouseout="this.className=\'' . $objlass . '\'" class="' . $objlass . '">
	  ...
	  <td align="center">' . $met_['noidentitas'] . '</td>
	  <td><a href="../aajk_report.php?er=_spk&ids='.$met_['id'].'" title="SPK untuk klient" target="_blank">'.strtoupper($met_['nama']).'</a></td>
	  <td align="center">' . _convertDate($met_['dob']) . '</td>
	  <td align="center">' . _convertDate($met_formspk['tgl_asuransi']) . '</td>
	  <td align="center">' . $met_['tenor'] . '</td>
	  <td align="center">' . _convertDate($met_formspk['tgl_akhir_asuransi']) . '</td>
	  <td align="center">' . $met_['mpp'] . '</td>
      ...
      <td align="center">' . $met_['approve_em_date'] . '</td>
...
LEFT JOIN fu_ajk_grupproduk ON fu_ajk_spak.id_mitra = fu_ajk_grupproduk.id
INNER JOIN fu_ajk_costumer ON fu_ajk_spak.id_cost = fu_ajk_costumer.id
WHERE (fu_ajk_spak.`status` = "Aktif" OR fu_ajk_spak.`status` = "Tolak" OR fu_ajk_spak.`status` = "Realisasi" OR fu_ajk_spak.`status` = "Kadaluarsa" OR fu_ajk_spak.`status` = "Batal") AND
	   fu_ajk_spak.del IS NULL AND
...
WHERE (fu_ajk_spak.`status` = "Aktif" OR fu_ajk_spak.`status` = "Tolak" OR fu_ajk_spak.`status` = "Realisasi"  OR fu_ajk_spak.`status` = "Kadaluarsa" OR fu_ajk_spak.`status` = "Batal") AND
	...
	$_REQUEST['x'] ? $pageNow = $_REQUEST['x'] : $pageNow = 1;
		while ($met_ = mysql_fetch_array($met)) {
			...
			if ($met_['status']=="Realisasi" or $met_['status']=="Batal") {
...
case "set_spak_mp":
    ...
    case "edit_percepatan":
        ...
        $spkdokter = mysql_fetch_array($database->doQuery('SELECT *, DATE_FORMAT(input_date,"%Y-%m-%d") AS tglinput FROM fu_ajk_spak WHERE id=' . $_REQUEST['ids'] . ''));
        $spkdokter_form = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_spak_form WHERE idspk=' . $_REQUEST['ids'] . ' AND id=' . $_REQUEST['idform'] . ' '));
        $polis = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_polis WHERE id = "'.$spkdokter['id_polis'].'"'));
        ...
        if ($_REQUEST['ope'] == "Simpan") {
          ...
          if ($_REQUEST['spk_dob'] == "")     {   $error_5 = '<font color="red"><blink>Silahkan isi tanggal lahir debitur.<br /></font>'; }
          if ($_REQUEST['spk_plafond'] == "") {   $error_12 = '<font color="red"><blink>Silahkan isi jumlah pinjaman.<br /></font>';  }


          //validasi penulisan plafond
          $spk_plafond = $_REQUEST['spk_plafond'];
          $spk_plafond = str_replace('.',"",$spk_plafond);
          $spk_plafond = str_replace(',',"",$spk_plafond);

          //validasi usia
          $met_Date = dateDiff($spkdokter_form['tgl_asuransi'], $_REQUEST['spk_dob']);
          $met_Date_ = explode(",", $met_Date);
          if ($met_Date_[1] >= 6) {
              $umur = $met_Date_[0] + 1;
          } else {
            ...
            if($spkdokter['id_polis'] == 11){
              if($spkdokter['danatalangan']==1){
                if($spkdokter_form['tenor'] <= 12){
                  $tenor = 1;
                }elseif($spkdokter_form['tenor'] > 12 and $spkdokter_form['tenor'] <= 24){
                  $tenor = 2;
                }else{
                  $tenor = 3;
                }
                ...
                $tenor = $spkdokter_form['tenor'];
              }
            }else{
              $tenor = $spkdokter_form['tenor'];
            }

          $agetenor = $polis['age_max']+1;

          //validasi usia masuk
          if($umur < $polis['age_min'] or $umur > $polis['age_max']){
            $error_5 = '<font color="red"><blink>Usia '.$umur.' Tidak Sesuai ketentuan polis ('.$polis['age_min'].' - '.$polis['age_max'].')<br /></font>';
          }elseif(($umur + $tenor) > $agetenor){
            $error_5 = '<font color="red"><blink>Usia '.$umur.' dengan tenor '.$tenor.' Tidak Sesuai ketentuan polis ('.$agetenor.')<br /></font>';
          }

          //validasi up max
          if($umur <= $polis['age_mid']){
            $maxup = $polis['up_max'];
          }else{
            $maxup = $polis['up_max2'];
          }

          if($_REQUEST['spk_plafond'] > $maxup){
            $error_12 = '<font color="red"><blink>Plafond '.duit($_REQUEST['spk_plafond']).' tidak sesuai ketentuan polis (Max. '.duit($maxup).').<br /></font>';
          }

          //set rate & premi
          $tenorrate = $spkdokter_form['tenor']*12;
          ...
          if($spkdokter['id_polis'] == 11){
            if($spkdokter['danatalangan']==1){
              ...
              $tenorrate = $spkdokter_form['tenor'];
            }
            $mpp = " AND '".$spkdokter_form['mpp']."' BETWEEN mpp_s and mpp_e";
          }

          //cek pasangan
          if($spkdokter['id_polis'] == 19){
            $pasangan = " AND usia = '".$umur."'";
            $tenorrate = $spkdokter_form['tenor'];
          }
          $rate = mysql_fetch_array($database->doQuery("SELECT *
                                                        FROM fu_ajk_ratepremi
                                                        WHERE id_polis = '".$spkdokter['id_polis']."' AND
                                                              tenor = '".$tenorrate."' AND
                                                              '".date("Y-m-d",strtotime($spkdokter['input_date']))."' BETWEEN eff_from and eff_to
                                                              ".$mpp."
                                                              ".$pasangan."
                                                              "));


          if($spkdokter_form['ratebank'] != $rate['rate']){
            $premi = ROUND(($_REQUEST['spk_plafond']/1000) * $rate['rate']);
            $qrate=  'ratebank="'.$rate['rate'].'",';
            $qpremi= 'x_premi = "'.$premi.'",';
          }elseif(ROUND(($_REQUEST['spk_plafond']/1000) * $rate['rate']) != $spkdokter_form['x_premi']){
            $premi = ROUND(($_REQUEST['spk_plafond']/1000) * $rate['rate']);
            $qpremi= 'x_premi = "'.$premi.'",';
          }else{
            $qrate="";
          ...
          if($error_1 or $error_2 or $error_3 or $error_4 or $error_5 or $error_12){
          }else{
            if(!isset($rate['rate'])){
              echo '<center><h2>Data Tidak bisa di ubah, harap hubungi IT';
            }else{
        ...
        if(isset($_REQUEST['spk_nama'])){
          $nama = $_REQUEST['spk_nama'];
        }else{
          $nama = $spkdokter_form['nama'];
        }

        if(isset($_REQUEST['spk_dob'])){
          $dob = $_REQUEST['spk_dob'];
        }else{
          $dob = $spkdokter_form['dob'];
        }

        if(isset($_REQUEST['spk_plafond'])){
          $plafond = $_REQUEST['spk_plafond'];
        }else{
          $plafond = $spkdokter_form['plafond'];
        }
         ...
         echo '<form name="f1" method="post" action="">
                <table border="0" cellpadding="1" cellspacing="1" width="85%" align="center">
                  ...
                  <tr><td>Nama<font color="red">*</font></td>
                      <td>: <input name="spk_nama" type="text" size="50" value="' . $nama . '" placeholder="Nama Peserta"> ' . $error_1 . '
                  </tr>
                  <tr><td>No Identitas</td>
                      <td>: <input name="noidentitas" type="text" size="50" value="' . $spkdokter_form['noidentitas'] . '" placeholder="Nomor Identitas">
                  </tr>
                  ...
                  <tr><td>Tanggal Lahir <font color="red">*</font></td>
                      <td>: <input type="text" name="spk_dob" id="rdob" class="tanggal" value="' . $dob . '" size="10"/> Usia :'.$umur.' ' . $error_5 . '
                      </td>
                  </tr>
                  <tr><td>Alamat <font color="red">*</font></td>
                      <td>: <textarea name="spk_alamat" type="text"rows="6" cols="45" placeholder="Alamat">' . $spkdokter_form['alamat'] . '</textarea> ' . $error_3 . '</td>
                  </tr>
                  <tr><td>Pekerjaan</td>
                      <td>: <input name="spk_pekerjaan" type="text" size="50" placeholder="Pekerjaan" value="' . $spkdokter_form['pekerjaan'] . '"> ' . $error_4 . ' </td>
                  </tr>
                  <tr><td width="20%">Jumlah Pinjaman/Kredit <font color="red">*</font></td>
                      <td>: <input type="text" name="spk_plafond" value="' . $plafond . '" size="30" id="inputku"  placeholder="Plafond" > ' . $error_12 . ' ' . $error_15 . ' ' . $error_16 . '</td>
                  <!--
