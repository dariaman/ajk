...
echo '<table border="0" cellpadding="3" cellspacing="0" width="100%">
     <tr><th width="95%" align="left">Setting Polis Asuransi</font></th>
     	 <th align="center"><a href="ajk_mspolis_as.php"><img border="0" src="image/Backward-64.png" width="25"></a></th>
     </tr>
     </table><br />';
if ($_REQUEST['ope']=="Simpan") {
	...
	{
		$rcomppolis = mysql_fetch_array($database->doQuery('SELECT id, id_cost, brokrage FROM fu_ajk_polis WHERE id="'.$_REQUEST['nmproduk'].'"'));
		$sisabrokrage = $rcomppolis['brokrage'] - $_REQUEST['share_as'];
		if ($sisabrokrage < 0) {	echo '<center>Share asuransi tidak mencukupi !</center>';	}
		else{
		$updatesharepolis = $database->doQuery('UPDATE fu_ajk_polis SET brokrage="'.$sisabrokrage.'" WHERE id="'.$_REQUEST['nmproduk'].'"');
		$rcomp = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_costumer WHERE id="'.$rcomppolis['id_cost'].'"'));

		$polis =mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_polis_as ORDER BY id DESC'));
		if ($polis['idp']=="") {	$x = 1;	}	else	{	$x = $polis['idp'] + 1;	}
...
$met = $database->doQuery('INSERT INTO fu_ajk_polis_as SET idp="'.$x.'",
												nopol="'.$RNoPolis.'",
												id_cost="'.$rcomppolis['id_cost'].'",
												id_as="'.$_REQUEST['name'].'",
												nmproduk="'.$_REQUEST['nmproduk'].'",
												share_as="'.$_REQUEST['share_as'].'",
												...
												input_date="'.$futgl.'"');
		echo '<blink><center>Nomor polis <b>'.$RNoPolis.'</b> telah di buat oleh <b>'.$_SESSION['nm_user'].'</b></center></blink><meta http-equiv="refresh" content="1; url=ajk_mspolis_as.php">';
		}
	}
}

echo '<form method="POST" action="" class="input-list style-1 smart-green">
		<h1>Setting Polis Asuransi Baru</h1>
		<table border="0" width="100%" cellpadding="1" cellspacing="1">
		<tr><td colspan="2">
			<label><span>Nama Asuransi <font color="red">*</font> '.$error1.'</span>
			   <select id="name" name="name">
			   <option value="">--- Pilih ---</option>';
		$comp = $database->doQuery('SELECT * FROM fu_ajk_asuransi ORDER BY name');
...
echo '</select>
		...
		<tr><td width="50%">
		<label><span>Nama Produk <font color="red">*</font> '.$error7.'</span>
			   <select id="nmproduk" name="nmproduk" onchange="FillTextBox(this.value)">
			   <option value="">--- Pilih ---</option>';
		$comp_prod = $database->doQuery('SELECT id, nmproduk, nopol, id_cost FROM fu_ajk_polis WHERE del IS NULL ORDER BY id_cost, id DESC');
		while ($comp_prod_ = mysql_fetch_array($comp_prod)) {
			if ($comp_prod_['nmproduk']=="") {	$namaproduk = $comp_prod_['nopol'];	}else{	$namaproduk = $comp_prod_['nmproduk'];	}
			$metBank = mysql_fetch_array($database->doQuery('SELECT id, name FROM fu_ajk_costumer WHERE id="'.$comp_prod_['id_cost'].'"'));
			echo '<option value="'.$comp_prod_['id'].'"'._selected($_REQUEST['nmproduk'], $comp_prod_['id']).'>'.$metBank['name'].' - '.$namaproduk.'</option>';
		}
echo '</select>
		</label>
		</td>
		<td>
		<label><span>Share Asuransi</span><input type="text" name="share_as" value="'.$_REQUEST['share_as'].'" placeholder="Share Asuransi (%)"></label>
		</td></tr>
		<tr><td colspan="2">
	   ...
	   <tr><td width="50%">
	   <label><span> </span><input type="hidden">
	   						<input type="radio" name="typePolis" value="openpolis"'.pilih($_REQUEST["openpolis"], "openpolis").' id="type_0">Open Policy
   		...
   		<label><span>Effective Date <font color="red">*</font> '.$error2.'</span>
		...
		<div id="Company_box">
			<label><span>Effective Date <font color="red">*</font> '.$error2.'</span><br />
	  		<table border="0" width="100%"><input type="hidden">
	   ...
	   <td valign="top">
		...
		<label><span>Single Rate : </span><input type="hidden">
	...
	case "view":
echo '<table border="0" cellpadding="3" cellspacing="0" width="100%">
	  <tr><th width="95%" align="left">Data Polis - View</font></th>
...
echo '<table border="0" width="100%" cellpadding="3" cellspacing="1" style="border: solid 1px #DEDEDE">
	...
	case "e_polis":
echo '<table border="0" cellpadding="3" cellspacing="0" width="100%">
	  <tr><th width="98%" align="left">Edit Nomor Polis</font></th>
		  <th align="center"><a href="ajk_mspolis_as.php"><img border="0" src="image/Backward-64.png" width="15"></a></th>
	  </tr>
	  </table><br />';
$met = mysql_fetch_array($database->doQuery('SELECT * FROM v_edit_polis WHERE id="'.$_REQUEST['idp'].'"'));
if ($_REQUEST['ope']=="Simpan") {
	if ($_REQUEST['typePolis']=="openpolis") {
		$_REQUEST['effdate'] = $_POST['effdate'];	if (!$_REQUEST['effdate'])  $error2 .='<blink><font color=red>Tentukan tanggal efektif polis</font></blink><br>';
	...
	{
		$rcomppolis = mysql_fetch_array($database->doQuery('SELECT id, id_cost, brokrage FROM fu_ajk_polis WHERE id="'.$met['nmproduk'].'"'));
		if ($rcomppolis['brokrage'] < 100) {	$ambilsisabrokrage = $rcomppolis['brokrage'] + $met['share_as'];
												$updatesharepolis = $database->doQuery('UPDATE fu_ajk_polis SET brokrage="'.$ambilsisabrokrage.'" WHERE id="'.$met['nmproduk'].'"');
		}	else	{	$ambilsisabrokrage = $rcomppolis['brokrage'];	}


		$sisabrokrage = $ambilsisabrokrage - $_REQUEST['share_as'];

		if ($sisabrokrage < 0) {	echo '<center>Share asuransi tidak mencukupi, share yang tersisa <b>'.$ambilsisabrokrage.'%</b> !</center>';	}
		else{
		$updatesharepolis_ = $database->doQuery('UPDATE fu_ajk_polis SET brokrage="'.$sisabrokrage.'" WHERE id="'.$met['nmproduk'].'"');

$met = $database->doQuery('UPDATE fu_ajk_polis_as SET polis_type="'.$_REQUEST['typePolis'].'",
													  nopol="'.$_REQUEST['er_nopol'].'",
													  share_as="'.$_REQUEST['share_as'].'",
													  polis_type="'.$_REQUEST['typePolis'].'",
													  noreferensi="'.$_REQUEST['er_noref'].'",
													  nokontrak="'.$_REQUEST['er_nokont'].'",
		...
		echo '<blink><center>Nomor polis <b>'.$met['nopol'].'</b> telah di edit oleh <b>'.$_SESSION['nm_user'].'</b></center></blink><meta http-equiv="refresh" content="1; url=ajk_mspolis_as.php">';
	}
	}
}
echo '<form method="POST" action="" class="input-list style-1 smart-green">
		...
		<tr><td colspan="2"><label><span>Nama Asuransi '.$met['namaasuransi'].' <br />Nomor Polis '.$met['nopol'].' <br /> Nama Perusahaan '.$met['namabank'].'</label></td></tr>
		<tr><td width="50%">
		<label><span>Nama Produk</span>
			   <select id="nmproduk" name="nmproduk" onchange="FillTextBox(this.value)">
			   <option value="">--- Pilih ---</option>';
		$comp_prod = $database->doQuery('SELECT id, nmproduk, nopol FROM fu_ajk_polis ORDER BY nmproduk DESC');
		while ($comp_prod_ = mysql_fetch_array($comp_prod)) {
			if ($comp_prod_['nmproduk']=="") {	$namaproduk = $comp_prod_['nopol'];	}else{	$namaproduk = $comp_prod_['nmproduk'];	}
			echo '<option value="'.$comp_prod_['id'].'"'._selected($met['nmproduk'], $comp_prod_['id']).' DISABLED>'.$namaproduk.'</option>';
		}
echo '</select>
		</label>
		</td>
		<td>
		<label><span>Share Asuransi</span><input type="text" name="share_as" value="'.$met['share_as'].'" placeholder="Share Asuransi (%)"></label>
		</td></tr>
		<tr><td colspan="2">
		<label><span>Nomor Polis Asuransi</span><input type="text" name="er_nopol" value="'.$met['nopol'].'" placeholder="Nomor Polis Asuransi"></label>
		<label><span>Nomor Referensi</span><input type="text" name="er_noref" value="'.$met['noreferensi'].'" placeholder="Nomor Referensi"></label>
		<label><span>Nomor Kontrak</span><input type="text" name="er_nokont" value="'.$met['nokontrak'].'" placeholder="Nomor Kontrak"></label>
	   </td></tr>
		<tr><td colspan="2" class="title2">Policy Rules</td></tr>
	   <tr><td width="50%">
	   <label><span> </span><input type="hidden">
	   						<input type="radio" name="typePolis" value="openpolis"'.pilih($met["polis_type"], "openpolis").' id="type_0">Open Policy
							<input type="radio" name="typePolis" value="closepolis"'.pilih($met["polis_type"], "closepolis").' id="type_1">Close Policy
		</label>
   		<div id="Individual_box">
   		<label><span>Effective Date <font color="red">*</font> '.$error2.'</span><br />
	   ...
	   <td valign="top">
		<label><span>Pengurangan Jumlah Hari (Akhir Kredit) : </span><input type="hidden">
																	 <input type="radio" name="dateminus" value="1"'.pilih($met["day_kredit"], "1").'>1 Hari
											 						 <input type="radio" name="dateminus" value="0"'.pilih($met["day_kredit"], "0").'>0 Hari
		</label><br />
		<label><span>Single Rate By Usia: </span><input type="hidden">
												 <input type="radio" name="singlerate" value="Y"'.pilih($met["singlerate"], "Y").'>Ya
										 		 <input type="radio" name="singlerate" value="T"'.pilih($met["singlerate"], "T").'>Tidak
		</label><br />
	   	...
	   	<label><span>PPN 23</span><input type="text" name="er_pph" value="'.$met['pph23'].'" placeholder="PPH 23 (%)" maxlength="2"></label>
		<label><span>Benefit Type <font color="red">*</font> '.$error3.'</span>
				<select size="1" name="benefit">
	   			<option value="">--- Pilih ---</option>
	   ...
	   <tr><td colspan="2" class="title2">Agent</td></tr>
	   <tr><td>
	   <label><span>Nama Penutup (GC / Agen)</span><input type="text" name="agentpenutup" value="'.$met['agentpenutup'].'" placeholder="Nama Penutup"></label>
	   <label><span>Nama Manager Group </span><input type="text" name="agentmanager" value="'.$met['agentmanager'].'" placeholder="Nama Manager"></label>
	   </td>
	   <td>
	   <label><span>Posisi Nama Penutup</span><select size="1" name="posisiagent">
	   			<option value="">--- Pilih ---</option>
	   			<option value="Manager"'._selected($met['posisiagent'],"Manager").'>Manager</option>
	   			<option value="Supervisor"'._selected($met['posisiagent'],"Supervisor").'>Supervisor</option>
	   			<option value="Staff"'._selected($met['posisiagent'],"Staff").'>Staff</option>
	   			<option value="Alt. Distribution"'._selected($met['posisiagent'],"Alt. Distribution").'>Alt. Distribution</option>
	   			</select></label>
	   <label><span>Posisi Nama Manager</span><select size="1" name="posisimngr">
	   			<option value="">--- Pilih ---</option>
	   			<option value="Manager"'._selected($met['posisimngr'],"Manager").'>Manager</option>
	   			<option value="Supervisor"'._selected($met['posisimngr'],"Supervisor").'>Supervisor</option>
	   			<option value="Staff"'._selected($met['posisimngr'],"Staff").'>Staff</option>
	   			<option value="Alt. Distribution"'._selected($met['posisimngr'],"Alt. Distribution").'>Alt. Distribution</option>
	   			</select></label>
	   </td>
	   </tr>
	   ...
	   <tr><td valign="top">
			...
			<label><span>Limit For Financial Statements</span><input type="text" name="limitfinancial" value="'.$met['limitfinancial'].'" placeholder="Batasan Laporan Keuangan"></label>
	   	   ...
	   	   <td valign="top">
	   	   	<table border="0" width="100%">
	   	   <tr><td><label><span>Bank DN</span><input type="text" name="bank1" value="'.$met['bank1'].'" placeholder="Nama Bank Debit Note"></label>
	   	   		   <label><span>Cabang</span><input type="text" name="cabang1" value="'.$met['cabang1'].'" placeholder="Cabang"></label>
	   	   		   <label><span>No. Rek</span><input type="text" name="norek1" value="'.$met['norek1'].'" placeholder="Nomor Rekening"></label>
		</td>
		<td><label><span>Bank CN</span><input type="text" name="bank2" value="'.$met['bank2'].'" placeholder="Nama Bank Credit Note"></label>
	   	   		   <label><span>Cabang</span><input type="text" name="cabang2" value="'.$met['cabang2'].'" placeholder="Cabang"></label>
	   	   		   <label><span>No. Rek</span><input type="text" name="norek2" value="'.$met['norek2'].'" placeholder="Nomor Rekening"></label>
		</td>
			</tr>
		   </table>
	   	   </td>
	...
	default:
echo '<table border="0" cellpadding="3" cellspacing="0" width="100%">
	  <tr><th width="95%" align="left">Data Polis</font></th>
     	  <th align="center"><a href="ajk_mspolis_as.php?op=newpol"><img src="image/new.png" border="0" width="25"></a></th>
     	  </tr>
   	  </table><br />';
echo '<table border="0" width="100%" cellpadding="3" cellspacing="1" style="border: solid 1px #DEDEDE">
	<!--<form method="post" action="">
	<tr><td width="10%">Nomor Polis</td><td>: <input type="text" name="nopol" value="'.$_REQUEST['nopol'].'"></td></tr>
	<tr><td width="10%">Produk</td><td>: <input type="text" name="produk" value="'.$_REQUEST['produk'].'"></td></tr>
	<tr><td colspan="2"><input type="submit" name="button" value="Cari" class="button"></td></tr>
	</form>-->
	<form method="post" action="">
	<tr><td width="10%">Nama Persusahaan</font></td>
...
echo '</select></td></tr>
	<tr><td>Nama Produk </td><td><select name="id_polis" id="id_polis"><option value="">--- Produk ---</option></select></td></tr>
	<tr><td colspan="2"><input type="submit" name="button" value="Cari" class="button"></td></tr>
	</form></table>
	</table>';
if ($_REQUEST['id_cost'])		{	$satu = 'AND fu_ajk_polis_as.id_cost =  "' . $_REQUEST['id_cost'] . '"';		}
if ($_REQUEST['id_polis'])		{	$dua  = 'AND fu_ajk_polis_as.nmproduk = "' . $_REQUEST['id_polis'] . '"';		}
echo '<table border="0" width="100%" cellpadding="4" cellspacing="1" bgcolor="#bde0e6">
	...
	<th width="3%">Disc%</th>
	<th width="8%">Max Age</th>
	<th width="1%">Option</th>
	</tr>';
//$metpolis = $database->doQuery('SELECT * FROM fu_ajk_polis_as WHERE id !="" '.$satu.' AND del IS NULL ORDER BY id_as, nmproduk DESC');
$metpolis = $database->doQuery('SELECT
...
fu_ajk_polis_as.noreferensi,
fu_ajk_polis_as.nokontrak,
fu_ajk_polis_as.share_as,
fu_ajk_polis_as.up_max,
fu_ajk_polis_as.polis_start,
fu_ajk_polis_as.polis_end,
fu_ajk_polis_as.discount,
fu_ajk_polis_as.benefit,
fu_ajk_polis_as.age_min,
fu_ajk_polis_as.age_max,
...
WHERE  fu_ajk_polis_as.id !="" '.$satu.' '.$dua.' AND fu_ajk_polis.del IS NULL AND fu_ajk_polis_as.del IS NULL ORDER BY fu_ajk_polis_as.id_as, fu_ajk_polis_as.id, fu_ajk_polis_as.id_cost DESC');
while ($met = mysql_fetch_array($metpolis)) {
/*
	$cekcost = mysql_fetch_array($database->doQuery('SELECT id, name FROM fu_ajk_costumer WHERE id= "'.$met['id_cost'].'" AND del IS NULL'));
	$cekprod = mysql_fetch_array($database->doQuery('SELECT id, id_cost, nmproduk FROM fu_ajk_polis WHERE id= "'.$met['nmproduk'].'" AND id_cost="'.$met['id_cost'].'" AND del IS NULL'));
	$cek_as = mysql_fetch_array($database->doQuery('SELECT id, name FROM fu_ajk_asuransi WHERE id= "'.$met['id_as'].'" AND del IS NULL'));
*/
...
echo '<tr onmouseover="this.className=\'tbl-over\'" onmouseout="this.className=\'' . $objlass . '\'" class="' . $objlass . '">
		...
		<td>'.$met['asuransi'].'</a></td>
		<td align="center"><a href="ajk_mspolis_as.php?op=view&id='.$met['id'].'">'.$met['nopol'].'</a></td>
		<td align="center">'.$met['produk'].'</td>
		<td>'.$met['client'].'</td>
		<td align="right">'.$met['share_as'].'% &nbsp;</td>
		...
		<td align="center">'.$met['age_min'].'-'.$met['age_max'].'</td>
		<td align="center"><a href="ajk_mspolis_as.php?op=e_polis&idp='.$met['id'].'"><img src="image/edit3.png"></a></td>
		</tr>';
}
		echo '</table>';
	;
