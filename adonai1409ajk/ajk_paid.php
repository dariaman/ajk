...
/*
			...
			if (strtoupper($cekIDPeserta['nama']) == strtoupper($data5)) {
			...
			if ($cekIDPeserta['totalpremi']) {
				$cekIDTotalpremi = duit($cekIDPeserta['totalpremi']);
			}else{
...
case "viewPaid_as2":
	...
	/*$cekIDPeserta = mysql_fetch_array($database->doQuery('UPDATE
				    `fu_ajk_note_as`
				    INNER JOIN `fu_ajk_cn`
				        ON (`fu_ajk_note_as`.`id_cn` = `fu_ajk_cn`.`id`)
					SET
				    `fu_ajk_cn`.`tgl_bayar_asuransi`=`fu_ajk_note_as`.`note_paid_date`
				    , `fu_ajk_cn`.`total_bayar_asuransi`=`fu_ajk_note_as`.`note_paid_total`
					WHERE `fu_ajk_cn`.`tgl_bayar_asuransi` IS NULL AND `fu_ajk_note_as`.`note_paid_date` IS NOT NULL
					'));
	*/
	mysql_query('UPDATE
...
echo '<table border="0" cellpadding="5" cellspacing="0" width="100%">
	  <tr><th align="left">Modul Upload Data Pembayaran perPeserta</th><th align="left" width="1%"><a href="ajk_paid.php?r=peserta"><img src="image/Backward-64.png" width="20"></a></th></tr>
	  </table>';
...
<tr><td width="10%">Nama Perusahaan</td>
		<td><select size="1" name="metCompany">
			...
			';
/*
$metCompany = $database->doQuery('SELECT * FROM appline_customer ORDER BY name ASC');
		while ($metCompany_ = mysql_fetch_array($metCompany)) {
			echo '<option value="' . $metCompany_['id'] . '"' . _selected($_REQUEST['metCompany'], $metCompany_['id']) . '>' . $metCompany_['name'] . '</option>';
		}
*/
//echo '</select></td></tr>

echo '<tr><td width="20%">Nomor Bukti Pembayaran Debitnote<font color="red">*</font></td><td><input type="text" name="nomorbukti" value="'.$_REQUEST['nomorbukti'].'" required></td></tr>
	<tr><td>File Pembayaran (.xls)<font color="red">*</font></td><td><input name="filepembayaran" type="file" accept="application/vnd.ms-excel" required></td></tr>
	<tr><td><input type="hidden" name="r" value="viewPaid" class="button"><input type="submit" name="button" value="Upload Pembayaran" class="button"></td></tr>
	</form>
	</table>';
		;
		break;
case "paidupload_as1":
	include_once ("../includes/functions.php");
			...
			echo '<table border="0" cellpadding="5" cellspacing="0" width="100%">
			  <tr><th align="left">Modul Upload Data Pembayaran perPeserta</th><th align="left" width="1%"><a href="ajk_claim.php"><img src="image/Backward-64.png" width="20"></a></th></tr>
			  </table>';
					echo '<table border="0" width="100%" cellpadding="3" cellspacing="1" style="border: solid 1px #DEDEDE" enctype="multipart/form-data">
			<form method="post" action="" enctype="multipart/form-data">
	...
	case "peserta":
echo '<table border="0" cellpadding="5" cellspacing="0" width="100%">
	  <tr><th align="left">Modul Upload Data Pembayaran perPeserta</th><th align="left" width="1%"><a href="ajk_paid.php?r=paidupload" title="upload data pembayaran perpeserta"><img src="image/rmf_2.png" width="25"></a></th></tr>
	  </table>';
if ($_REQUEST['e']=="paid") {
