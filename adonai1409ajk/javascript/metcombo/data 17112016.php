...
switch($req) {
	...
	case "setpolisasuransi": // city
		...
		$sql = "SELECT fu_ajk_asuransi.id, fu_ajk_asuransi.`name`, fu_ajk_polis_as.nopol
				...
				WHERE fu_ajk_polis_as.id_cost = ".$_POST["id_cost"]." AND fu_ajk_polis_as.del IS NULL
   ...
   $sql = "SELECT fu_ajk_asuransi.id, fu_ajk_asuransi.`name`, fu_ajk_polis_as.nopol
   FROM fu_ajk_asuransi
   INNER JOIN fu_ajk_polis_as ON fu_ajk_asuransi.id = fu_ajk_polis_as.id_as where fu_ajk_polis_as.nmproduk='".$_POST["id_polis"]."' AND fu_ajk_polis_as.del IS NULL
   ORDER BY fu_ajk_asuransi.`name` ASC";
		}
		break;
	case "setasuransi":
