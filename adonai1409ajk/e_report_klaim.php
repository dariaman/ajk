...
case "klaim_share":
	...
	case "klaim_share1" :
			...
			for ($kol_tahun = 2012; $kol_tahun <= date('Y'); $kol_tahun++) {
					$premi=0;
					$jml=0;
					$klaim=0;
					...
					$pdf->ln();
					$pdf->SetX(5);
					$pdf->Cell(15,5,$kol_tahun,1,0,'C');
					for ($me = $_GET['y_uw']; $me <= ($_GET['y_uw']+2); $me++){
						$sqlku="SELECT
						...
						and YEAR(fu_ajk_peserta.kredit_tgl)=".$kol_tahun."
						and YEAR(fu_ajk_cn.tgl_claim)=".$me."
						and fu_ajk_costumer.id=".$_REQUEST['id_cost']." ".$prod."
						GROUP BY
						...
						$row=$rows*65;
						$pdf->SetX(20+$row);
						$pdf->Cell(20,5,number_format($hasil['totalpremi'],2),1,0,'R');
						$pdf->SetX(40+$row);
						$pdf->Cell(20,5,number_format($hasil['jml'],0),1,0,'C');
						$pdf->SetX(60+$row);
						$pdf->Cell(25,5,number_format($hasil['tuntutan_klaim'],2),1,0,'R');
						
						$premi+=$hasil['totalpremi'];
						$jml+=$hasil['jml'];
						$klaim+=$hasil['tuntutan_klaim'];
					...
					$row=$rows*65;
					$pdf->SetX(20+$row);
					$pdf->Cell(20,5,number_format($premi,2),1,0,'R');
					$pdf->SetX(40+$row);
					$pdf->Cell(20,5,number_format($jml,0),1,0,'C');
					$pdf->SetX(60+$row);
					$pdf->Cell(25,5,number_format($klaim,2),1,0,'R');
			...
			for ($kol_tahun = $_GET['y_uw']; $kol_tahun <= date('Y'); $kol_tahun++) {
				if($rows<3){
					...
					$datanya=mysql_fetch_array($result);
					
					$row=$rows*65;
					$pdf->SetX(20+$row);
					$pdf->Cell(20,5,number_format($datanya['totalpremi'],2),1,0,'R');
					$pdf->SetX(40+$row);
					$pdf->Cell(20,5,number_format($datanya['jml'],0),1,0,'C');
					$pdf->SetX(60+$row);
					$pdf->Cell(25,5,number_format($datanya['tuntutan_klaim'],2),1,0,'R');
					$rows++;
					
					$premi+=$datanya['totalpremi'];
					$jml+=$datanya['jml'];
					$klaim+=$datanya['tuntutan_klaim'];
				}
			}
	...
	case "akadvsdol" :
		...
		$pdf->SetX($row);
		$pdf->Cell(20,10,'UW Year',1,0,'C');
		$pdf->SetX($baris);
		$pdf->Cell(100,5,'DOL',1,0,'C');
		$pdf->ln();
		
		$qdol = mysql_query("select year(dol)as tgl_dol from rpt_klaim group by year(dol)");

		while($qdol_ = mysql_fetch_array($qdol)){
			$pdf->SetX($baris);
			$pdf->Cell(20,5,$qdol_['tgl_dol'],1,0,'C');
			$baris = $baris + 20;
		}
		
		$quwyear = mysql_query("select year(tgl_akad)as tgl_akad from rpt_klaim group by year(tgl_akad)");
		while($quwyear_ = mysql_fetch_array($quwyear)){
			$pdf->ln();
			$pdf->SetX($row);
			$pdf->Cell(20,10,$quwyear_['tgl_akad'],1,0,'C');
			/*
			$qdol2 = mysql_query("select year(dol)as tgl_dol from rpt_klaim group by year(dol)");

			$baris = $row+20;
			
			while($qdol2_ = mysql_fetch_array($qdol2)){	
				$qval = mysql_fetch_array(mysql_query("select count(*)as jml from rpt_klaim where year(tgl_akad) = '".$quwyear_['tgl_akad']."' and year(dol) = '".$qdol2_['tgl_dol']."'"));
				$pdf->SetX($baris);
				$pdf->Cell(20,5,$qval['jml'],1,0,'C');
				$baris = $baris + 20;
			}
			*/
		}			
...
case "_invklaim":
	...
	class PDF extends FPDF
	{
		...
		function Footer()
		{
			...
			$pdf->Ln(3);
			$pdf->setFont('Arial','',10);
			$pdf->Cell(0,4,"Analisis Claim Analyst Adonai",0,1,'L',false);
			$pdf->Ln(1);
			$pdf->setFont('Arial','',8);
...
case "_analisaklaim":
	...
	class PDF extends FPDF
	{
		...
		function Header()
		{
			...
			//$this->Image('../image/logo_adonai.png',50,10,-300);
			$this->Image('../image/logo_adonai.png', 50, 5, 100);
		}
		...
		function Footer()
		{
			...
			$met_klaim_polis = mysql_fetch_array(mysql_query('SELECT id, id_cost, nopol, nmproduk, bank_2, cabang_2, rek_2 FROM fu_ajk_polis WHERE id="'.$met_klaim['id_nopol'].'" AND id_cost="'.$met_klaim['id_cost'].'" AND del IS NULL'));
			$met_klaim_peserta = mysql_fetch_array(mysql_query('SELECT *, IF(type_data="SPK", kredit_tenor * 12, kredit_tenor) AS kredit_tenor FROM fu_ajk_peserta WHERE id_cost="'.$met_klaim['id_cost'].'" AND id_polis="'.$met_klaim['id_nopol'].'" AND id_peserta="'.$met_klaim['id_peserta'].'" AND id_dn="'.$met_klaim['id_dn'].'" AND id_klaim="'.$met_klaim['id'].'" AND status_peserta="Death" AND del IS NULL'));
			$met_klaim_dn = mysql_fetch_array(mysql_query('SELECT id, id_cost, id_nopol,id_as, dn_kode, dn_status, tgl_dn_paid FROM fu_ajk_dn WHERE id_cost="'.$met_klaim['id_cost'].'" AND id_nopol="'.$met_klaim['id_nopol'].'" AND id="'.$met_klaim['id_dn'].'" AND del IS NULL'));
			$met_penyakit = mysql_fetch_array(mysql_query('SELECT * FROM fu_ajk_namapenyakit WHERE id="'.$met_klaim['nmpenyakit'].'"'));
			$met_analisa = mysql_fetch_array(mysql_query('SELECT * FROM fu_ajk_analisa_klaim WHERE id_peserta="'.$met_klaim['id_peserta'].'" and del is null'.$tipe));
			$met_asuransi = mysql_fetch_array(mysql_query('SELECT * FROM fu_ajk_asuransi WHERE id = "'.$met_klaim_dn['id_as'].'"'));
			//if ($met_klaim['confirm_claim']!="Approve(paid)") {	$headtagihan = $pdf->Text(83, 40,'MEDICAL OPINION');	}
			//else{	$headtagihan = $pdf->Text(83, 40,'MEDICAL OPINION');	}
			$mets =datediff($met_klaim_peserta['kredit_tgl'], $met_klaim['tgl_claim']);
			$usiapolis = explode(",", $mets);
			...
			$pdf->Text(10, 95,'Tenor');				$pdf->Text(50, 95,': '.$met_klaim_peserta['kredit_tenor'].' Bulan');
			if($_REQUEST['tp'] == "bank"){
				$pdf->Text(10, 100,'Penyebab Meninggal');$pdf->Text(50, 100,': '.$met_penyakit['namapenyakit'].' - '.$met_analisa['ext_penyebab']);	
			}
			$pdf->Text(135, 55,'Nama Produk');		$pdf->Text(165, 55,': '.$met_klaim_polis['nmproduk']);
			$pdf->Text(135, 60,'Tanggal DOL');	$pdf->Text(165, 60,': '._convertDate($met_klaim['tgl_claim']));
			$pdf->Text(135, 65,'Usia Polis');		$pdf->Text(165, 65,': '.$usiapolis[0].' Tahun '.$usiapolis[1].' Bulan '.$usiapolis[2].' Hari');
			if($_REQUEST['tp'] == "as"){
				$pdf->Text(135, 70,'Asuransi');$pdf->Text(165, 70,': '.$met_asuransi['code']);	
			}			
			...
			$pdf->SetFillColor(200,220,255);
			$pdf->Cell(0,6,$judul." OPINION",0,1,'C',true);

			$pdf->Ln(5);
			$pdf->setFont('Arial','',10);
			$pdf->Cell(0,4,"Hasil Analisa",0,1,'L',false);
			$pdf->Ln(1);
			$pdf->setFont('Arial','',8);
			if($_REQUEST['tp'] == "bank"){
				$pdf->MultiCell(0,6,$met_analisa['analisa_bank'],1);
			}else{
				$pdf->MultiCell(0,6,$met_analisa['analisa_asuransi'],1);
			}
			$pdf->Output();
			;
break;
