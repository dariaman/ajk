...
echo '<tr onmouseover="this.className=\'tbl-over\'" onmouseout="this.className=\'' . $objlass . '\'" class="' . $objlass . '">
	...
	case "refund":
echo '<table border="0" cellpadding="5" cellspacing="0" width="100%"><tr><th width="95%" align="left">Modul Laporan Rekapitulasi Refund</font></th></tr></table>';
$metcost = $database->doQuery('SELECT * FROM fu_ajk_costumer ORDER BY name DESC');
echo '<form method="post" action="" name="postform">
	  <input type="hidden" name="id_cost" value="'.$metcost['id'].'">
	  <table border="0" cellpadding="1" cellspacing="0" width="100%">
...
echo '</select></td></tr>
	  ...
	  <tr><td align="right">Tanggal Refund</td>
...
echo '</td></tr>
	  <tr><td align="right">Tanggal Cetak CN</td>
	  	  <td> :';print initCalendar();	print calendarBox('tglcheck3', 'triger3', $_REQUEST['tglcheck3']);	echo 's/d';
...
if ($_REQUEST['x']) {	$m = ($_REQUEST['x']-1) * 25;	}	else {	$m = 0;		}
$met = $database->doQuery('SELECT * FROM fu_ajk_cn WHERE id_cn !="" AND type_claim="Refund" AND del is NULL '.$satu.' '.$duaa.' '.$tiga.' '.$empt.' ORDER BY tgl_createcn ASC LIMIT '. $m .' , 25');
$totalRows = mysql_fetch_array($database->doQuery('SELECT COUNT(id) FROM fu_ajk_cn WHERE id !="" AND type_claim="Refund" AND del is NULL '.$satu.' '.$duaa.' '.$tiga.' '.$empt.''));
$totalRows = $totalRows[0];
$_REQUEST['x'] ? $pageNow = $_REQUEST['x'] : $pageNow = 1;
while ($met_ = mysql_fetch_array($met)) {
$met_peserta = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_peserta WHERE id_cost="'.$met_['id_cost'].'" AND id_polis="'.$met_['id_nopol'].'" AND id_peserta="'.$met_['id_peserta'].'"'));
...
if ($_REQUEST['id_cost']=="") 	{	$error_1 = '<div align="center"><font color="red"><blink>Silahkan pilih nama perusahaan...!!<br /></div></font></blink>';	}
if ($_REQUEST['tglcheck3']=="") 	{	$error_2 = '<div align="center"><font color="red"><blink>Tanggal mulai mulai tidak boleh kosong<br /></div></font></blink>';	}
if ($_REQUEST['tglcheck4']=="") 	{	$error_3 = '<div align="center"><font color="red"><blink>Tanggal akhir mulai tidak boleh kosong</div></font></blink>';	}
if ($error_1 OR $error_2 OR $error_3) {	echo $error_1 .''.$error_2.''.$error_3;	}
else{
echo '<table border="0" width="100%" cellpadding="1" cellspacing="1"  bgcolor="#E2E2E2">
		<tr>
			<!-- <td bgcolor="#FFF"colspan="1"><a href="e_report.php?er=eL_klaim&cat='.$_REQUEST['id_cost'].'&subcat='.$_REQUEST['id_polis'].'&tgl1='.$_REQUEST['tglcheck1'].'&tgl2='.$_REQUEST['tglcheck2'].'&tgl3='.$_REQUEST['tglcheck3'].'&tgl4='.$_REQUEST['tglcheck4'].'"><img src="image/excel.png" width="25" border="0"><br />Excel</a></td> -->
			<td bgcolor="#FFF"colspan="23"><a href="e_report.php?er=eL_klaim&cat='.$_REQUEST['id_cost'].'&subcat='.$_REQUEST['id_polis'].'&tgl1='.$_REQUEST['tglcheck1'].'&tgl2='.$_REQUEST['tglcheck2'].'&tgl3='.$_REQUEST['tglcheck3'].'&tgl4='.$_REQUEST['tglcheck4'].'"><img src="image/excel.png" width="25" border="0"><br />Download Klaim</a></td>
		</tr>
			<th width="1%">No</th>
			<th>Cabang</th>
...
if ($_REQUEST['tglcheck3'])		{	$empt = 'AND fu_ajk_cn.tgl_createcn BETWEEN "'.$_REQUEST['tglcheck3'].'" AND "'.$_REQUEST['tglcheck4'].'" ';	}
if ($_REQUEST['x']) {	$m = ($_REQUEST['x']-1) * 25;	}	else {	$m = 0;		}
//$met = $database->doQuery('SELECT * FROM fu_ajk_cn WHERE id_cn !="" AND type_claim="Death" AND del is NULL '.$satu.' '.$duaa.' '.$tiga.' '.$empt.' ORDER BY tgl_createcn ASC LIMIT '.$m.', 25');
...
INNER JOIN fu_ajk_dn ON fu_ajk_cn.id_dn = fu_ajk_dn.id AND fu_ajk_cn.id_cost = fu_ajk_dn.id_cost AND fu_ajk_cn.id_nopol = fu_ajk_dn.id_nopol
INNER JOIN fu_ajk_peserta ON fu_ajk_cn.id_cost = fu_ajk_peserta.id_cost AND fu_ajk_cn.id_nopol = fu_ajk_peserta.id_polis AND fu_ajk_cn.id_dn = fu_ajk_peserta.id_dn AND fu_ajk_cn.id_peserta = fu_ajk_peserta.id_peserta
LEFT JOIN fu_ajk_asuransi ON fu_ajk_asuransi.id = fu_ajk_dn.id_as
WHERE fu_ajk_cn.type_claim = "Death" AND fu_ajk_cn.del IS NULL '.$satu.' '.$duaa.' '.$tiga.' '.$empt.' LIMIT '.$m.', 25');
$totalRows = mysql_fetch_array($database->doQuery('SELECT COUNT(id) FROM fu_ajk_cn WHERE id !="" AND type_claim="Death" AND del is NULL '.$satu.' '.$duaa.' '.$tiga.' '.$empt.''));
$totalRows = $totalRows[0];
$_REQUEST['x'] ? $pageNow = $_REQUEST['x'] : $pageNow = 1;
while ($met_ = mysql_fetch_array($met)) {
