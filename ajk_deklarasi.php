...
switch ($_REQUEST['hn']) {
    ...
    case "confirm_deklarasi": //EXECUTE DEKLARASI STAFF
        if (!$_REQUEST['nameRef']) {
            ...
            foreach ($_REQUEST['nameRef'] as $r => $id) {
                ...
                $validasi = mysql_fetch_array(mysql_query("SELECT * FROM fu_ajk_age_rent_val WHERE idp = '".$query['id_polis']."'"));

                if ($query['plafond'] > $validasi['plafond_max']) {
                    $error = $error.'<br>'.$query['nama'].'('.$query['spak'].') Plafond melebihi batas maksimum plafond, data tidak bisa di proses.';
                }
                ...
                $hasil = $query['x_usia']+$tenorth;
                //validasi usia
                if ($hasil > $validasi['age_max']+1) {
                    //$error = $error.'<br>'.$query['nama'].'('.$query['spak'].') Usia '.$hasil.' thn melebihi batas maksimum usia, data tidak bisa di proses.';
                }
                ...
                $ext_premi = ROUND($query['x_premi'] * ($query['ext_premi'] / 100));
                $totalpremi = ROUND($query['x_premi'] + $ext_premi);

                $nama = str_replace("'", "\\'", $query['nama']);
                //cek data peserta yang sudah masuk
                if (mysql_num_rows(mysql_query("select * from fu_ajk_peserta_tempf where spaj = '".$query['spak']."' and del is null")) > 0) {
                } else {
                    if ($q['cabang']=="JAKARTA") {
                        $cabang = "cabang='JAKARTA',cabangreal='".$query['nm_cabang']."',";
                    } else {
                        $cabang = "cabang='".$query['nm_cabang']."',";
                    }
    ...
    case "edit": //EXECUTE EDIT
        ...
        $spk = mysql_fetch_array(mysql_query("SELECT fu_ajk_spak_form.dob,
                                                 ...
                                                 date_format(fu_ajk_spak.approve_date,'%Y-%m-%d')as approve_date
																					FROM fu_ajk_spak
																							 INNER JOIN fu_ajk_spak_form
																							 ON fu_ajk_spak_form.idspk = fu_ajk_spak.id
																					WHERE idspk =".$id_spk));
        // echo $spk['approve_date'].' '.$tgl_asuransi;
        if(($spk['id_polis'] == 1 or $spk['id_polis'] == 12) and $spk['approve_date'] > $tgl_asuransi){
          $pesan = '<br><br><center>Update Gagal! Tanggal Akad Tidak Bisa lebih kecil dari tanggal Approve </center><meta http-equiv="refresh" content="5; url=ajk_deklarasi.php?hn=editspk&id='.$id_spk.'">';                    
        // }elseif(($spk['id_polis'] != 1 or $spk['id_polis'] != 12) and $spk['input_date'] > $tgl_asuransi){
        //   $pesan = '<br><br><center>Update Gagal! Tanggal Akad Tidak Bisa lebih kecil dari tanggal Input </center><meta http-equiv="refresh" content="5; url=ajk_deklarasi.php?hn=editspk&id='.$id_spk.'">';          
        }else{
          ...
          $ratebank = mysql_fetch_array(mysql_query("SELECT * FROM fu_ajk_ratepremi WHERE id_polis = '".$spk['id_polis']."' and tenor = '".$tenor."' ".$mpptype." ".$qpumur." AND date_format('".$tgl_asuransi."','%Y-%m-%d') BETWEEN eff_from and eff_to"));
          ...
          $validasi = mysql_fetch_array(mysql_query("SELECT * FROM fu_ajk_age_rent_val WHERE idp = '".$spk['id_polis']."'"));
          $hasil = $umur+$tenor;
          //validasi usia
          if ($hasil > $validasi['age_max']+1) {
              //$error = $error.'Usia '.$hasil.' thn melebihi batas masksimum usia, data tidak bisa di edit.';
          }
          ...
          if ($error != "") {
              $pesan = '<br><br><center>'.$error.'</center><meta http-equiv="refresh" content="5; url=ajk_deklarasi.php?hn=editspk&id='.$id_spk.'">';
          } else {
              mysql_query($query) or die("<br><br>Error message = ".mysql_error().$query);
              $pesan = '<br><br><center>Data Telah Diubah</center><meta http-equiv="refresh" content="5; url=ajk_deklarasi.php">';
          }
        }
        echo $pesan;
    break;
    ...
    case "editspk":	//FORM EDIT
        $spk = mysql_fetch_array(mysql_query("SELECT * FROM fu_ajk_spak WHERE id = ".$_REQUEST['id']));
        ...
        echo'	<br><table border="0" cellpadding="5" cellspacing="0" width="100%">
								<tr>
									<th width="95%" align="left">Edit ['.$qspk['nama'].' - '.$spk['spak'].']</font></th>
								</tr>
							</table>';
    ...
    case "spaj_appr": //EXECUTE DEKLARASI SPV
        if (!$_REQUEST['idtemp']) {
            ...
            foreach ($_REQUEST['idtemp'] as $k => $val) {
                ...
                //cek data peserta yang sudah masuk
                if (mysql_num_rows(mysql_query("select * from fu_ajk_peserta where spaj = '".$qpeserta['spaj']."' and status_aktif = 'Approve' and del is null")) > 0) {
                } else {
                    $mameto = $database->doQuery('INSERT INTO fu_ajk_peserta
																				SET 	id_cost="'.$qpeserta['id_cost'].'",
																							...
																							no_ktp="'.$qpeserta['no_ktp'].'",
																							pekerjaan="'.$qpeserta['pekerjaan'].'",
                                                                                            ...
                                                                                            cabangreal="'.$qpeserta['cabangreal'].'",
																							...
																							nopermohonan="'.$qpeserta['nopermohonan'].'",
    ...
    default: //DEKLARASI STAFF FORM
        ...
        if ($_REQUEST['re']=="datadeklarasi") {
            ...
            if ($q['cabang']=='JAKARTA') {
                ...
                $filterspak = "fu_ajk_spak.spak not in (SELECT spaj FROM fu_ajk_peserta_tempf where del is null) AND " ;
            }
            ...
            if ($flag_spak == 1 or $flag_cabang == 1) {
                $cabang='';
            } else {
            ...
            while ($qdeklarasi_row = mysql_fetch_array($qdeklarasi)) {
                ...
                // deklarasi cabang cekbox di hide jika dibuka di jakarta
                if ($q['cabang']=='JAKARTA') {
                    $q_temp = mysql_fetch_array(mysql_query("SELECT count(*)as jml FROM fu_ajk_peserta_tempf where del is null and spaj = '".$qdeklarasi_row['spak']."'"));

                    if ($q_temp['jml'] > 0) {
                        $cekbox = '' ;
                    } else {
                        $cekbox = '<input type="checkbox" class="case" name="nameRef[]" value="'.$qdeklarasi_row['id_spak'].'">';
                    }
                } else {
                    $cekbox = '<input type="checkbox" class="case" name="nameRef[]" value="'.$qdeklarasi_row['id_spak'].'">';
                }
