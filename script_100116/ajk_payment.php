...
if ($_REQUEST['ope']=="Simpan") {
	...
	else
	{
		...
		echo ('UPDATE v_fu_ajk_peserta SET spaj="'.$_REQUEST['rspaj'].'",
												   			 nama="'.$_REQUEST['rnama'].'",
...
$reg = $database->doQuery('SELECT * FROM v_fu_ajk_peserta WHERE id!="" '.$querydata1.' AND id_cost="'.$q['id_cost'].'" AND status_aktif="pending" ORDER BY spaj ASC');
while ($creg = mysql_fetch_array($reg)) {
if ($creg['spaj']=="") {	$spajnya = '(IDpeserta) '.$creg['id_peserta'];	}else{	$spajnya = '(SPAJ) '.$creg['spaj'];	}
	echo '<option value="'.$creg['spaj'].'">'.$spajnya.'</option>';	}
echo '</select></td>
   		</tr>
...
$data = $database->doQuery('SELECT * FROM v_fu_ajk_peserta WHERE id!="" '.$querydata1.' AND id_cost="'.$q['id_cost'].'" AND status_aktif="pending" '.$satu.' '.$dua.' AND del IS NULL ORDER BY cabang ASC');
...
if ($fpos === false) {
	...
	$fmedic = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_medical_form WHERE idp="'.$fudata['id'].'"'));
	if ($fmedic['idp'] == $fudata['id'])
	{	$docmedik = '<a title="Download file" href="ajk_file/medical/'.$fmedic['file_medical'].'"><img src="image/re_file_dl.png" width="20"></a>';	}	else
	{	$docmedik = '<font color="blue">'.$fudata['status_medik'].'</font>';	}
...
echo '<fieldset style="padding: 2">
	...
	<table border="0" width="20%" cellpadding="1" cellspacing="1" align="center">
		...
		<tr><td align="right">Nomor SPAJ :</td>
      		<td><select size="1" name="sspaj">
	   		<option value="">- Pilih SPAJ -</option>';
$reg = $database->doQuery('SELECT * FROM v_fu_ajk_peserta WHERE id_cost="'.$q['id_cost'].'" '.$querydata1.' AND status_aktif="pending" ORDER BY spaj ASC');
while ($creg = mysql_fetch_array($reg)) {
if ($creg['spaj']=="") {	$spajnya = '(IDpeserta) '.$creg['id_peserta'];	}else{	$spajnya = '(SPAJ) '.$creg['spaj'];	}
echo '<option value="'.$creg['spaj'].'">'.$spajnya.'</option>';
}
...
$data = $database->doQuery('SELECT * FROM v_fu_ajk_peserta WHERE id!="" '.$querydata1.' AND id_cost="'.$q['id_cost'].'" AND status_aktif="pending" AND status_medik="NM" '.$satu.' '.$dua.' AND del IS NULL ORDER BY cabang ASC');
while ($fudata = mysql_fetch_array($data)) {
echo '<tr class="'.rowClass(++$i).'">
		  <td align="center">'.++$no.'</td>
		  <td>'.$fudata['spaj'].'</td>
		  ...
		  <td align="right">'.duit($fudata['totalpremi']).'</td>
		  <td align="center">'.$fudata['statement1'].' | '.$fudata['statement2'].' | '.$fudata['statement3'].' | '.$fudata['statement4'].'</td>
		  <td align="center"><b>'.$fudata['status_aktif'].'</b></td>
...
if ($_REQUEST['rspaj'])		{	$satu = 'AND spaj LIKE "%' . $_REQUEST['rspaj'] . '%"';	}
if ($_REQUEST['rnama'])		{	$dua = 'AND nama LIKE "%' . $_REQUEST['rnama'] . '%"';	}
$riweuh = explode("-", $_REQUEST['rdob']);	$cektgldn = $riweuh[2].'/'.$riweuh[1].'/'.$riweuh[0];
if ($_REQUEST['rdob'])		{	$tiga = 'AND tgl_lahir LIKE "%'.$cektgldn.'%"';	}

$rcarireg = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_regional WHERE id="'.$_REQUEST['id_reg'].'"'));	if ($_REQUEST['id_reg'])	{	$empat = 'AND regional LIKE "%' . $rcarireg['name'] . '%"';	}
$rcariare = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_area WHERE id="'.$_REQUEST['id_area'].'"'));	if ($_REQUEST['id_area'])		{	$lima = 'AND area LIKE "%' . $rcariare['name'] . '%"';	}
$rcaricab = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_cabang WHERE id="'.$_REQUEST['id_cabang'].'"'));	if ($_REQUEST['id_cabang'])	{	$enam = 'AND cabang LIKE "%' . $rcaricab['name'] . '%"';	}

if ($_REQUEST['x']) {	$m = ($_REQUEST['x']-1) * 25;	}
else {	$m = 0;		}
if ($_REQUEST['ope']=="paid") {	$met = $database->doQuery('SELECT * FROM fu_ajk_peserta WHERE id != "" '.$querydata1.' AND id_cost="'.$q['id_cost'].'" AND status_bayar="1" '.$satu.' '.$dua.' '.$tiga.' '.$empat.' '.$limat.' '.$enam.' ORDER BY input_time DESC, cabang ASC, id DESC LIMIT ' . $m . ' , 25');
	$totalRows = mysql_fetch_array($database->doQuery('SELECT COUNT(id) FROM fu_ajk_peserta WHERE id != "" '.$querydata1.' AND id_cost="'.$q['id_cost'].'" AND status_bayar="1" ' . $satu . ' '.$dua.' '.$tiga.' '.$empat.' '.$limat.' '.$enam.' ORDER BY input_time DESC, cabang ASC, id DESC'));
	$totalRows = $totalRows[0];
}
elseif ($_REQUEST['ope']=="unpaid") {	$met = $database->doQuery('SELECT * FROM fu_ajk_peserta WHERE id != "" '.$querydata1.' AND id_cost="'.$q['id_cost'].'" AND status_bayar="0" '.$satu.' '.$dua.' '.$tiga.' '.$empat.' '.$limat.' '.$enam.' ORDER BY input_time DESC, cabang ASC, id DESC LIMIT ' . $m . ' , 25');
	$totalRows = mysql_fetch_array($database->doQuery('SELECT COUNT(id) FROM fu_ajk_peserta WHERE id != "" '.$querydata1.' AND id_cost="'.$q['id_cost'].'" AND status_bayar="0" ' . $satu . ' '.$dua.' '.$tiga.' '.$empat.' '.$limat.' '.$enam.' ORDER BY input_time DESC, cabang ASC, id DESC'));
	$totalRows = $totalRows[0];
}else{
	$met = $database->doQuery('SELECT * FROM fu_ajk_peserta WHERE id != "" AND status_aktif!="maturity"  '.$querydata1.' AND id_cost="'.$q['id_cost'].'" '.$satu.' '.$dua.' '.$tiga.' '.$empat.' '.$limat.' '.$enam.' ORDER BY kredit_tgl DESC LIMIT ' . $m . ' , 25');
	$totalRows = mysql_fetch_array($database->doQuery('SELECT COUNT(id) FROM fu_ajk_peserta WHERE id != "" AND status_aktif!="maturity" '.$querydata1.' AND id_cost="'.$q['id_cost'].'" ' . $satu . ' '.$dua.' '.$tiga.' '.$empat.' '.$limat.' '.$enam.''));
	$totalRows = $totalRows[0];
}
$metz = $met;
...
//if ($fu['status_peserta']=="") {	$metstatuspeserta = 'Aktif';	}	else {	$metstatuspeserta = $fu['status_peserta'];	}
...
if ($fu['spaj']=="") {	$spaknya = $fu['nama_mitra'];	}else{	$spaknya = $fu['spaj'];	}
echo '<tr onmouseover="this.className=\'tbl-over\'" onmouseout="this.className=\'' . $objlass . '\'" class="' . $objlass . '">
	  	  <td align="center">'.$fudata['input_time'].' '.$metedit.'</td>
	  	  <td align="center">'.(++$no + ($pageNow-1) * 25).'</td>
		  <td align="center">'.$spaknya.'</td>
		  <td>'.$fu['nama'].'</td>
		  ...
		  <td align="right">'.$fu['kredit_akhir'].'</td>
		  <td align="right">'.duit($fu['kredit_jumlah']).'</td>
		  <td align="right">'.duit($fu['totalpremi']).'</td>
		  <td align="center">'.$statusnya.'</td>
		  <td align="center">'.$fu['id_dn'].'</td>
		  <td align="center">'.$fu['status_aktif'].'</td>
		  <td>'.$fu['ket'].'</td>
		  <td>'.$fu['cabang'].'</td>
 		  <td>'.$fu['area'].'</td>
		  <td>'.$fu['regional'].'</td>
...
echo createPageNavigations($file = 'ajk_payment.php?fu=detailpay&id_reg='.$_REQUEST['id_reg'].'&id_area='.$_REQUEST['id_area'].'&id_cabang='.$_REQUEST['id_cabang'].'&ope='.$_REQUEST['ope'].'&', $total = $totalRows, $psDeh = 10 , $anchor = '', $perPage = 25);
...
echo '	<tr><th width="1%" rowspan="2">No</th>
			...
			<th width="1%" rowspan="2">Bunga<br>%</th>
			<th width="1%" rowspan="2">Premi</th>
			<th width="1%" rowspan="2">Diskon</th>
			<th colspan="3">Biaya</th>
			<th width="1%" rowspan="2">Total Premi</th>
			<th rowspan="2">Medical</th>
		</tr>
...
$data = $database->doQuery('SELECT * FROM fu_ajk_peserta WHERE id!="" AND id_dn="'.$fusdn['dn_kode'].'" AND del is null ORDER BY cabang ASC');
while ($fudata = mysql_fetch_array($data)) {
...
echo '<tr class="'.rowClass(++$i).'">
		  ...
		  <td align="center">'.$fudata['kredit_tgl'].'</td>
		  <td align="center">'.$fudata['kredit_tenor'].'</td>
		  <td align="center">'.$tanggalplus.'</td>
		  <td align="right">'.duit($fudata['kredit_jumlah']).'</td>
				<td align="center">'.$fudata['bunga'].'</td>
				<td align="right">'.duit($fudata['premi']).'</td>
...
echo '<tr><th colspan="12">Total</th>
			  	  ...
			  	  <th>'.duit($jpremi).'</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th>
...
<form method="post" action="" name="frmcust" onSubmit="return valcust()">
<tr><td width="5%">Regional</td>
	<td width="15%">: <select name="id_reg" id="id_reg">
	<option value="">-- Select Regional --</option>';
		$rows = mysql_query('select * from fu_ajk_regional WHERE id!="" AND id_cost="'.$q['id_cost'].'" '.$querydata.'');
		while($row = mysql_fetch_array($rows)) {
			$sel = ""; if ($_POST["id_reg"] == $row["id"]) $sel = ' selected="selected"';
...
echo '</select></td>
		<td width="8%">DN Date </td><td>: ';print initCalendar();	print calendarBox('rdns', 'triger', $_REQUEST['rdns']);echo ' s/d ';
...
<tr><td>Area</td>
		...
		<td>Payment Date</td><td>: ';print initCalendar();	print calendarBox('rpays', 'triger2', $_REQUEST['rpays']); echo ' s/d ';
...
echo '</td></tr>
		...
		<option value="">-- Select Cabang --</option>
	  	  ...
	  	  <td>Nomor DN</td><td>: <input type="text" name="dns" value="'.$_REQUEST['dns'].'"> s/d <input type="text" name="dne" value="'.$_REQUEST['dne'].'">';
echo '</tr>
	  <tr><td colspan="2"><td>Payment Status</td>
...
$cekreg = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_regional WHERE id="'.$_REQUEST['id_reg'].'"'));	if ($_REQUEST['id_reg'])	{	$satu = 'AND id_regional LIKE "%'.$cekreg['name'].'%"';		}
$cekare = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_area WHERE id="'.$_REQUEST['id_area'].'"'));		if ($_REQUEST['id_area'])	{	$dua = 'AND id_area LIKE "%'.$cekare['name'].'%"';		}
$cekcab = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_cabang WHERE id="'.$_REQUEST['id_cabang'].'"'));	if ($_REQUEST['id_cabang'])	{	$tiga = 'AND id_cabang LIKE "%'.$cekcab['name'].'%"';		}
if ($_REQUEST['rdns']!='' AND $_REQUEST['rdne']!='')	{	$empat = 'AND tgl_createdn BETWEEN \''.$_REQUEST['rdns'].'\' AND \''.$_REQUEST['rdne'].'\'';		}
if ($_REQUEST['rpays']!='' AND $_REQUEST['rpaye']!='')	{	$lima  = 'AND tgl_dn_paid BETWEEN \''.$_REQUEST['rpays'].'\' AND \''.$_REQUEST['rpaye'].'\'';		}
if ($_REQUEST['dns']!='' AND $_REQUEST['dne']!='')		{	$enam = 'AND dn_kode BETWEEN "'.$_REQUEST['dns'].'" AND "'.$_REQUEST['dne'].'"';		}
if ($_REQUEST['rstat'])									{	$tujuh = 'AND dn_status LIKE "%' .$_REQUEST['rstat'] . '%"';	}
...
$metdn = $database->doQuery('SELECT * FROM fu_ajk_dn WHERE id!="" '.$querydata2.' AND id_cost="'.$q['id_cost'].'" '.$satu.' '.$dua.' '.$tiga.' '.$empat.' '.$lima.' '.$enam.' '.$tujuh.' AND del IS NULL ORDER BY dn_kode DESC, tgl_createdn DESC LIMIT ' . $m . ' , 25');
$totalRows = mysql_fetch_array($database->doQuery('SELECT COUNT(id) FROM fu_ajk_dn WHERE id!="" '.$querydata2.' AND id_cost="'.$q['id_cost'].'" '.$satu.' '.$dua.' '.$tiga.' '.$empat.' '.$lima.' '.$enam.' '.$tujuh.' AND del IS NULL'));
$totalRows = $totalRows[0];
...
if ($rmetdn['dn_status']=="") {	$dnpaid = '<font color="red"><blink>Unpaid</blink></font>';	}
else	{	$dnpaid = '<font color="blue">'.$rmetdn['dn_status'].'</font>';	}

$metcbg = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_peserta WHERE id_dn="'.$rmetdn['dn_kode'].'" AND id_cost="'.$q['id_cost'].'" AND del is null '));
$jpeserta = $database->doQuery('SELECT * FROM fu_ajk_peserta WHERE id_dn="'.$rmetdn['dn_kode'].'" AND id_cost="'.$q['id_cost'].'" AND del is null ');
...
//CEK DATA WILAYAH LAMA DENGAN YANG BARU
if ($rmetdn['id_regional']=="") { $cregional = $rmetdn['id_regional_old'];	}	else	{	$cregional = $rmetdn['id_regional'];	}
if ($rmetdn['id_area']=="") 	{ $carea = $rmetdn['id_area_old'];	}			else	{	$carea = $rmetdn['id_area'];	}
if ($rmetdn['id_cabang']=="") 	{ $ccabang = $rmetdn['id_cabang_old'];	}		else	{	$ccabang = $rmetdn['id_cabang'];	}
//CEK DATA WILAYAH LAMA DENGAN YANG BARU

$tglskrg = explode (" ", $futgl);					$tglsekarang = $tglskrg[0];
$tglinpt = explode (" ", $rmetdn['input_time']);	$tglinput	 = $tglinpt[0];
...
fu_ajk_cn
WHERE
fu_ajk_cn.id_dn = "'.$rmetdn['dn_kode'].'" AND id_cost="'.$rmetdn['id_cost'].'" AND fu_ajk_cn.del IS NULL
GROUP BY
fu_ajk_cn.id_dn'));
if ($dncnnya['id_dn']==$rmetdn['dn_kode']) {
	$cnnomor = '<a href="ajk_report.php?fu=ajkpdfcn&id_cn='.$dncnnya['id_cn'].'" target="_blank">'.$dncnnya['id_cn'].'</a>';
...
$netpremi = $rmetdn['totalpremi'] - $dncnnya['tclaim'];		//NILAI NETT PREMI
echo '<tr class="'.rowClass(++$i).'">
		  <td align="center">'.(++$no + ($pageNow-1) * 25).'</td>
		  <td><input name="ajkdn[]" type="hidden" id="checkbox[]" value="'.$rmetdn['id'].'" checked><b><a href="ajk_payment.php?fu=vmember&id='.$rmetdn['id'].'">'.$rmetdn['dn_kode'].'</a></b>'.$datenow.'</td>
		  ...
		  <td align="center">'.duit($netpremi).'</td>
		  <td align="center">'.$statuscn.'</td>
		  <td><b>('.$idcabang['nmr_cbg'].')</b> - '.$ccabang.'</td>
		  <td>'.$carea.'</td>
		  <td>'.$cregional.'</td>
		  <td align="center"><a href="ajk_report.php?fu=ajkpdfinvdn&id='.$rmetdn['id'].'&s='.$q['id'].'" title="DN Member '.$rmetdn['dn_kode'].'" target="_blank"><img src="image/dninvoice.png" width="21"></a></td>
		  </tr>';
}
echo '<tr><td colspan="8">';
echo createPageNavigations($file = 'ajk_payment.php?rdns='.$_REQUEST['rdns'].'&rdne='.$_REQUEST['rdne'].'&rpays='.$_REQUEST['rpays'].'&rpaye='.$_REQUEST['rpaye'].'&cat='.$_REQUEST['cat'].'&subcat='.$_REQUEST['subcat'].'&rdnno='.$_REQUEST['rdnno'].'&rstat='.$_REQUEST['rstat'].'&', $total = $totalRows, $psDeh = 10 , $anchor = '', $perPage = 25);
