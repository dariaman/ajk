...
if ($metFormSPK['filefotodebiturdua']!="") {
	$mamet_photonya = '<a href="../ajkmobilescript/'.$metFormSPK['filefotodebiturdua'].'" rel="lightbox" ><img src="../ajkmobilescript/'.$metFormSPK['filefotodebiturdua'].'" width="165"></a>';
}
else{
	$mamet_photonya = '<img src="image/non-user.png" width="175">';
}
echo '<table border="0" width="80%" cellpadding="3" cellspacing="1" align="center">
	  <tr><td width="20%">Nama Upload SPK</td><td colspan="2">: '.strtoupper($metFormSPK['dokter']).'</td></tr>
	  <tr><th colspan="3" bgcolor="#DEDEDE">Data Nasabah</th><t/tr>
	  <tr><td>Nama Nasabah</td><td>: <b>'.strtoupper($metFormSPK['nama']).'</b></td><td width="15%" rowspan="6" valign="top" align="center">'.$mamet_photonya.'</tr>
...
echo '<tr><th colspan="3" bgcolor="#DEDEDE">Data Peminjaman Asuransi</th><t/tr>
	  ...
	  <tr><td>Tenor</td><td colspan="2">: '.$metFormSPK['tenor'].'</td></tr>
	  ...
	  <tr><td>Cabang</td><td colspan="2">: '.$metFormSPK['cabang'].'</td></tr>
...
echo $quer1['nmproduk'].' ('.$quer1['nopol'].')';
		...
		$metProduk = $database->doQuery('SELECT * FROM fu_ajk_polis WHERE id_cost="'.$q['id_cost'].'" AND typeproduk="SPK"');
echo '<select name="polisnya">
		<option value="">--- Pilih ---</option>';
		while($metProduk_ = mysql_fetch_array($metProduk)) {
echo  '<option value="'.$metProduk_['id'].'"'._selected($_REQUEST['polisnya'], $metProduk_['id']).'>'.$metProduk_['nmproduk'].'</option>';
		}
echo '</select>';
		$kolomregional .= '<tr><td align="right">Regional :</td>
		<td>'.$q['wilayah'].'</td></tr>';
...
if ($_REQUEST['statusnya'])							{	$dua = 'AND status "' . $_REQUEST['statusnya'] . '"';		}
if ($_REQUEST['polisnya'])							{	$tiga = 'AND id_polis = "' . $_REQUEST['polisnya'] . '"';		}

if ($_REQUEST['x']) {	$m = ($_REQUEST['x']-1) * 25;	}	else {	$m = 0;		}
//$met = $database->doQuery('SELECT * FROM fu_ajk_spak WHERE id_cost="'.$q['id_cost'].'" AND id_polis="'.$q['id_polis'].'" '.$view_user.' '.$satu.' '.$dua.' '.$tiga.' AND  status!="Proses" AND status!="Approve" AND del IS NULL ORDER BY id DESC LIMIT ' . $m . ' , 25');	111315
$met = $database->doQuery('SELECT * FROM fu_ajk_spak WHERE id_cost="'.$q['id_cost'].'" '.$view_user.' '.$satu.' '.$dua.' '.$tiga.' AND  status!="Proses" AND status!="Approve" AND del IS NULL ORDER BY id DESC LIMIT ' . $m . ' , 25');
//$totalRows = mysql_fetch_array($database->doQuery('SELECT COUNT(id) FROM fu_ajk_spak WHERE id !="" AND  id_cost="'.$q['id_cost'].'" AND id_polis="'.$q['id_polis'].'"  '.$view_user.' '.$satu.' '.$dua.' '.$tiga.' AND  status!="Proses" AND status!="Approve" AND del IS NULL'));	111315
$totalRows = mysql_fetch_array($database->doQuery('SELECT COUNT(id) FROM fu_ajk_spak WHERE id !="" AND  id_cost="'.$q['id_cost'].'"'.$view_user.' '.$satu.' '.$dua.' '.$tiga.' AND  status!="Proses" AND status!="Approve" AND del IS NULL'));
