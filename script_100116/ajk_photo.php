...
if ($errno) {	echo '<tr><td colspan="4" align="center"><font color="red">'.$errno.'</font></td></tr>';	}
	...
	$photomet1 = $met_peserta['id_peserta'].'_'.$_FILES["photofile1"]["name"];
	$photomet2 = $met_peserta['id_peserta'].'_'.$_FILES["photofile2"]["name"];
...
$data = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_peserta WHERE nama !="" AND id_cost="'.$q['id_cost'].'" AND id_polis="'.$q['id_polis'].'" ' . $satu . ' '.$dua.' AND status_aktif!="Batal" AND del IS NULL'));
...
$caridata = mysql_num_rows($database->doQuery('SELECT * FROM fu_ajk_peserta WHERE nama !="" AND id_cost="'.$q['id_cost'].'" AND id_polis="'.$q['id_polis'].'" ' . $satu . ' '.$dua.' AND status_aktif!="Batal" AND del IS NULL'));
if ($caridata <= 0) {	echo '<center><font color="red">Maaf, data yang anda cari tidak ada. !</font></center>';	}else{
$met_photo = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_photo WHERE id_cost="'.$data['id_cost'].'" AND id_peserta="'.$data['id_peserta'].'"'));
