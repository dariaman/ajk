...
$fupolis = mysql_fetch_array($database->doQuery('SELECT id, nmproduk, age_deviasi FROM fu_ajk_polis WHERE id="'.$q['id_polis'].'"'));
...
echo '<form method="post" action="" onload ="onbeforeunload">
	  <table border="0" width="100%" cellpadding="1" cellspacing="1" style="border: solid 3px #DEDEDE"  bgcolor="#bde0e6">
		...
		<tr><th width="1%" rowspan="2">No</th>
			...
			<th width="10%" rowspan="2">Regional</th>
...
if ($data9 <= 9) { $data9_ = '0'.$data9;	}else{	$data9_ = $data9;}
	...
	elseif ($cekdataspknama['nama'] == $data3 AND $cekdataspknama['dob']!=$datatgllahirnya) {$error ='<font color="red">error tgl lahir</font>'; $dataexcel3=$error;}
	elseif ($cekdataspknama['nama'] == $data3 AND $cekdataspknama['dob']==$datatgllahirnya AND $cekdataspknama['idspk']!=$cekdataspk['id']) {$error ='<font color="red">error nomor SPK</font>'; $dataexcel3=$error;}
...
if(is_numeric($cekdataspknama['cabang'])){
	...
	$cekdatawilayah = mysql_fetch_array($database->doQuery('SELECT * FROM v_wilayah WHERE id_cost="'.$q['id_cost'].'" AND regional="'.$data13.'" AND area="'.$data14.'" AND cabang="'.$TabCabang__.'"'));
	if ($data13=="") {	$error ='<font color="red">error</font>'; $dataexcel13=$error;
	}else{ if ($cekdatawilayah['regional']!=$data13) {$error ='<font color="red">error</font>'; $dataexcel13=$error;}else{ $dataexcel13=$data13;}
	}
