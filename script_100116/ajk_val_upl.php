...
*/
	if ($metProduknya['mpptype']=="Y") {
		if ($fudata['mppbln'] <= 6) {
			$cekratepolis = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_ratepremi WHERE id_cost="'.$fudata['id_cost'].'" AND id_polis="'.$fudata['id_polis'].'" AND tenor="'.$fudata['kredit_tenor'].'" AND status="lama"'));		// RATE PREMI MENUNGGU PERSETUJUAN DARI BUKOPIN
		}else{
			$cekratepolis = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_ratepremi WHERE id_cost="'.$fudata['id_cost'].'" AND id_polis="'.$fudata['id_polis'].'" AND tenor="'.$fudata['kredit_tenor'] * 12 .'" AND '.$fudata['mppbln'].' BETWEEN mpp_s AND mpp_e AND status="baru"'));	//VALIDASI CEK TENOR DENGAN SETTING RATE POLIS
		}
	}
	else{
		$cekratepolis = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_ratepremi WHERE id_cost="'.$fudata['id_cost'].'" AND id_polis="'.$fudata['id_polis'].'" AND usia="'.$fudata['usia'].'" AND tenor="'.$fudata['kredit_tenor'].'" AND status="baru"'));		// RATE PREMI
	}
...
*/
	if ($metProduknya['mpptype']=="Y") {
		...
		$cekratepolis = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_ratepremi WHERE id_cost="'.$fudata['id_cost'].'" AND id_polis="'.$fudata['id_polis'].'" AND tenor="'.$fudata['kredit_tenor'].'" AND status="baru"'));		// RATE PREMI
	}
	$ppremistandar = $fudata['kredit_jumlah'] * $cekratepolis['rate'] / 1000;
...
//DATA VIEW STAFF
$data = $database->doQuery('SELECT * FROM fu_ajk_peserta_tempf WHERE nama !="" AND id_cost="'.$q['id_cost'].'" AND input_by="'.$q['nm_user'].'" AND status_aktif="Upload" AND del IS NULL ORDER BY input_time DESC, namafile ASC, no_urut ASC');
echo '<form method="post" action="" onload ="onbeforeunload">
	  <table border="0" width="100%" cellpadding="1" cellspacing="1" bgcolor="#bde0e6">
	<tr><th width="1%"></th>
		<th width="1%"><input type="checkbox" id="selectall"/></th>
...
if ($_REQUEST['el']=="_photo") {
	...
	echo '<div class="title2" align="center">Photo peserta telah berhasil di upload oleh <b>'.$q['nm_lengkap'].'</b> pada tanggal '.$futgldn.' !</div><meta http-equiv="refresh" content="3; url=ajk_val_upl.php?v=spaj">';
	}
}
...
foreach($_REQUEST['nama'] as $k => $val){
	...
	$admpolis = mysql_fetch_array($database->doQuery('SELECT id_cost, typeproduk, adminfee, day_kredit, discount, singlerate, min_premium, mpptype, mppbln FROM fu_ajk_polis WHERE id_cost="'.$r['id_cost'].'" AND id="'.$r['id_polis'].'"'));
	...
	if ($admpolis['typeproduk']=="SPK") {
		if ($admpolis['mpptype']=="Y") {
			if ($r['mppbln'] <= 6) {
				$cekrate = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_ratepremi WHERE id_cost="'.$r['id_cost'].'" AND id_polis="'.$r['id_polis'].'" AND tenor="'.$r['kredit_tenor'].'" AND status="lama"'));		// RATE PREMI MENUNGGU PERSETUJUAN DARI BUKOPIN
			}else{
				$cekrate = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_ratepremi WHERE id_cost="'.$r['id_cost'].'" AND id_polis="'.$r['id_polis'].'" AND tenor="'.$r['kredit_tenor'] * 12 .'" AND '.$r['mppbln'].' BETWEEN mpp_s AND mpp_e AND status="baru"'));	//VALIDASI CEK TENOR DENGAN SETTING RATE POLIS
			}
		}
		else{
			$cekrate = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_ratepremi WHERE id_cost="'.$r['id_cost'].'" AND id_polis="'.$r['id_polis'].'" AND usia="'.$r['usia'].'" AND tenor="'.$r['kredit_tenor'].'" AND status="baru"'));		// RATE PREMI
		}
		...
		if ($admpolis['mpptype']=="Y") {
			if ($r['mppbln'] <= 6) {
				...
				$cekrate = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_ratepremi WHERE id_cost="'.$r['id_cost'].'" AND id_polis="'.$r['id_polis'].'" AND tenor="'.$r['kredit_tenor'].'" AND '.$r['mppbln'].' BETWEEN mpp_s AND mpp_e AND status="baru"'));	//VALIDASI CEK TENOR DENGAN SETTING RATE POLIS
			}
		}else{
...
echo '<tr onmouseover="this.className=\'tbl-over\'" onmouseout="this.className=\'' . $objlass . '\'" class="' . $objlass . '">
		  ...
		  <td align="center">'.duit($fudata['mppbln']).'</td>
		  <td align="center">'.$fudata['cabang'].'</td>
		  <td align="center"><font color="blue"><b>'.$fudata['input_by'].'</b></font></td>
		  </tr>';
}
