...
$central=mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_cabang WHERE del is null and id = '.$mb['cabang']));
if(isset($_SESSION['timeout']) ) {
	$session_life = time() - $_SESSION['timeout'];
	if($session_life > $inactive) echo "0";	else echo "1";
}
...
echo "<div id='cssmenu'>
		  <ul><li><a href='imob.php'><span>Home</span></a></li>
			  ...
			  <li><a href='imob.php?ob=rSPK'><span>Report</span></a></li>";
 				if($central['centralcbg']!="" or $mb['namamitra']!= 1 or $mb['cabang']== 4){
...
switch ($_REQUEST['ob']) {
	...
	case "addrevisi":
		...
		$cabang = $central['name'];
		$qspv = mysql_fetch_array(mysql_query("select * from pengguna where nm_user = '".$mb['spv']."'"));
		...
		$footer = 'Demikian informasi yang dapat kami sampaikan.<br><br>Regards, <br><br>'.$mb['nm_user'];
		$tomail = $qspv['email'];
	...
	case "appspk":
		echo '
		<form method="post" action="imob.php?ob=appspkspv">
			...
			*/
			...
			case when fu_ajk_spak.status in ("Assign","Propose","Redirect") then "Approve" else fu_ajk_spak.status end as status,
			...
			while ($mobUserSPK_ = mysql_fetch_array($mobUserSPK)) {
				...
				<tr onmouseover="this.className=\'tbl-over\'" onmouseout="this.className=\'' . $objlass . '\'" class="' . $objlass . '">
					<td align="center"><a href="imob.php?ob=btlspk&id='.$mobUserSPK_['idspaktemp'].'&idu='.$mb['id'].'&x_spk=mobdel" onClick="if(confirm(\'Apakah anda yakin untuk menghapus data SPK ini ?\')){return true;}{return false;}"><img src="image/delete.gif"></a>&nbsp;
	...
	case "btlspk":
		...
		echo '<form method="post" action="ajk_val_upl.php?v=spk_del&id='.$_REQUEST['id'].'&x_spk=mobdel">
			  <table border="0" width="100%" cellpadding="1" cellspacing="1">
			  <input type="hidden" name="idspv" value="'.$mb['id'].'">
	...
	case "edtspk":
			$met = mysql_fetch_array($database->doQuery('SELECT * FROM fu_ajk_spak_form_temp WHERE id="'.$_REQUEST['id'].'"'));
			$metSPK = mysql_fetch_array($database->doQuery('SELECT id, id_cost, id_polis, spak,danatalangan FROM fu_ajk_spak WHERE id="'.$met['idspk'].'"'));
			...
			echo ''.$erroredit.'<form name="f1" method="post" enctype="multipart/form-data" action="">
			      ...
			      <!--<tr><td>Jangka Waktu Pinjaman <font color="red">*</font></td><td> : <input type="text" name="mettenor" value="'.$met['tenor'].'" onkeyup="this.value=this.value.replace(/[^0-9/,]/g,\'\')" maxlength="2" size="1"> tahun '.$error_8.'</td></tr>-->';
					...
					if($metSPK['danatalangan']!=1){
						echo '<tr><td>Jangka Waktu Pinjaman <font color="red">*</font></td><td> : <input type="text" name="mettenor" value="'.$met['tenor'].'" onkeyup="this.value=this.value.replace(/[^0-9/,]/g,\'\')" maxlength="2" size="5"> tahun '.$error_8.'</td></tr>';
					}
				  echo '
		...
		if (!$_REQUEST['nama']) {
			...
			foreach($_REQUEST['nama'] as $k => $val){
				...
				if (!$cekFormSPK['idspk']) {
					...
					$nama = str_replace('"', "", $metFormSPK['nama']);
					//$nama = str_replace("'", "\'", $nama);

					$mpppremi = ROUND($metFormSPK['plafond'] * $_ratevar / 1000);		//PREMI
			...
			while ($mailStaff_ = mysql_fetch_array($mailStaff)) {
				...
				if(mysql_num_rows($cekplatinum)>0){
					...
					$mail2->Subject = "AJKOnline - Cek Data Platinum";
				...
				/* cek pasangan */
				$cekpasangan = mysql_query("select *,plafond/1000*ratebank_sys as premi
																		from(
																		select fu_ajk_spak.id,
																						spak,
																						fu_ajk_spak.`status`,
																						ROUND(ratebank,2)as ratebank,
																						tenor,
																						plafond,
																						(select ROUND(rate,2) from fu_ajk_ratepremi where id_polis = 19 and fu_ajk_ratepremi.usia = fu_ajk_spak_form.x_usia and status = 'baru' and fu_ajk_ratepremi.tenor = fu_ajk_spak_form.tenor)as ratebank_sys,fu_ajk_spak.input_date
																		from fu_ajk_spak
																				 inner join fu_ajk_spak_form
																				 on fu_ajk_spak_form.idspk = fu_ajk_spak.id
																		where fu_ajk_spak.del is null and
																		...
																		where ratebank != ratebank_sys and id not in (68628,73516,73852);");

				if(mysql_num_rows($cekpasangan)>0){
					while($cekpasangan_ = mysql_fetch_array($cekpasangan)){
						$query = "UPDATE fu_ajk_spak_form SET ratebank = '".$cekpasangan_['ratebank_sys']."',x_premi = '".$cekpasangan_['premi']."' WHERE idspk = '".$cekpasangan_['id']."'";
						mysql_query($query);

						$message = $message."Cek Data Pasangan ".$cekpasangan_['spak'].' rate '.$cekpasangan_['ratebank'];
					}
					...
					$mail2->SMTPSecure = "ssl";
					$mail2->SetFrom ($mb['email'], $mb['namalengkap']);
					$mail2->Subject = "AJKOnline - Cek Data Pasangan";
					$mail2->AddAddress('hansen@adonai.co.id');
